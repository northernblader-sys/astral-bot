/**
 * <prefix>gojo-spin <amount>
 *
 * Gojo's spin. Same shared odds curve (chanceForExclusiveSpin), batch reel and
 * pity bar as Yato's, and the same spinOnly ownership model: NOT one-of-one, no
 * season lock, never for sale, everyone who clears the dead zone gets their own
 * copy (see "spinOnly" in data/characters.json and handleBuy() in
 * plugins/character.js). It differs from Yato in one way:
 *
 *   Priced at 1.5 gems per spin (Yato is 1). Everything else about the paid
 *   GEMS model is identical, and neither one has a fame wall.
 *
 * Odds curve (chanceForExclusiveSpin, with overrides):
 *   spins 1-180  -> 0%    DEAD_ZONE_UNTIL = 180, a true dead zone. He cannot be
 *                         won at all before spin 181.
 *   spin 181+    -> 80%   PLATEAU_CHANCE, flat per-spin, sustained to the cap.
 *                         PITY_AT sits far past the 230 cap so the guaranteed
 *                         win branch inside chanceForExclusiveSpin() never fires:
 *                         spin 230 is the same flat roll as spin 181, and there
 *                         is no hard guarantee anywhere on the curve.
 *
 * 230 spins lifetime. The first live roll costs 180 spins of dead zone, so
 * 270 gems in, and the whole run costs 345 gems. None of those numbers are ever
 * printed in player facing copy. Only the pity bar (progress toward the cap) is,
 * the same rule every spin in the bot follows: never reveal DEAD_ZONE_UNTIL or
 * PLATEAU_CHANCE.
 *
 * The banner ships behind the owner spin lock (lib/spin-locks.js): until the
 * owner runs `.unlockspin gojo`, spinLockGate() below refuses every pull.
 */
import { config } from '../config.js'
import { chanceForExclusiveSpin } from '../lib/season-engine.js'
import { characterMap } from '../lib/game-data.js'
import { updatePlayer } from '../lib/player-repo.js'
import { roundGems, fmtGems } from '../lib/format.js'
import { characterStars } from '../lib/rarity.js'
import { spinLockGate } from '../lib/spin-locks.js'

const CHARACTER_ID = 'gojo'
const SPIN_FIELD = 'gojoSpins'
const COST_PER_SPIN = 1.5       // gems per spin
const DEAD_ZONE_UNTIL = 180     // spins 1-180: 0%
const PLATEAU_CHANCE = 0.80     // spin 181+: flat 80%, sustained, no pity spike
const PITY_AT = 9999            // far past MAX_SPINS, so the guaranteed-win branch
                                // in chanceForExclusiveSpin() can never fire
const MAX_SPINS_PER_PLAYER = 230 // hard lifetime cap
// 20 rather than the 5 the Solar spins use. A 180 spin dead zone at 5 per
// command would be 36 commands before a first live roll, which reads as broken
// rather than a grind. 20 still fits the reel (rows of 10) and stops one giant
// pull becoming a wall of text.
const MAX_SPINS_PER_COMMAND = 20

const RULE = '━━━━━━━━━━━━━━━━━━━━'

const MISS_LINES = [
  'The distance never closes. Your reach dies before it lands.',
  'He does not move. He does not need to.',
  'Space thickens between you and him, and swallows the swing.',
  'The Six Eyes track you, unimpressed, and look away.',
  'Infinity holds. Whatever you threw is still travelling, and always will be.',
  'He has both hands in his pockets. That is the whole fight.',
]

function missLine(spin) {
  return `♾️ _${MISS_LINES[spin % MISS_LINES.length]}_`
}

/** Progress toward the spin cap. Never reveals the plateau. */
function pityBar(spin, width = 10) {
  const filled = Math.max(0, Math.min(width, Math.round((spin / MAX_SPINS_PER_PLAYER) * width)))
  return `${'▰'.repeat(filled)}${'▱'.repeat(width - filled)}`
}

function reel(results) {
  const MAX_ROWS = 5
  const marks = results.map(r => (r.won ? '✦' : '·'))
  const rows = []
  for (let i = 0; i < marks.length; i += 10) rows.push(marks.slice(i, i + 10).join(' '))
  if (rows.length <= MAX_ROWS) return rows.join('\n')
  return [`_…${(rows.length - MAX_ROWS) * 10} earlier spins_`, ...rows.slice(-MAX_ROWS)].join('\n')
}

async function replyOverArt(ctx, character, text) {
  if (!character?.image) return ctx.reply(text)
  try {
    return await ctx.replyImage(character.image, text)
  } catch {
    return ctx.reply(text)
  }
}

export default {
  name: 'gojo-spin',
  aliases: ['gojo', 'gojospin', 'go-spin', 'satoru', 'gojo-claim'],
  category: 'character',
  requiresPlayer: true,
  description: 'Spin for Gojo at 1.5 gems per spin. Never for sale',

  async run(ctx) {
    const p = config.prefix
    if (!ctx.player) return ctx.reply(`⚠️ Register first with *${p}register*.`)

    const character = characterMap[CHARACTER_ID]
    if (!character) return ctx.reply(`⚠️ Gojo isn't configured yet.`)

    // Owner spin lock. Nothing is read or spent past this until it is open.
    if (spinLockGate(ctx, CHARACTER_ID, character.name)) return

    if ((ctx.player.ownedCharacters ?? []).includes(CHARACTER_ID)) {
      return replyOverArt(ctx, character,
        `♾️ *${character.name} is already yours.*\n${RULE}\n` +
        `_Equip him with *${p}character equip ${CHARACTER_ID}*. Infinity guards you on its own, ` +
        `and *${p}hollowpurple* and *${p}unlimitedvoid* are yours once per battle each._`)
    }

    // First ever attempt, read before this command mutates the spin count. Only
    // decides whether a losing reply carries the art as a one time preview.
    const isFirstEverAttempt = (ctx.player[SPIN_FIELD] ?? 0) === 0

    const requested = Math.floor(Number(ctx.args[0]) || 1)
    const spinCount = Math.min(MAX_SPINS_PER_COMMAND, MAX_SPINS_PER_PLAYER, Math.max(1, requested))

    let outcome = null
    await updatePlayer(ctx.db, ctx.from, (player) => {
      // Re-checked inside the write-serialized mutator. The checks above are
      // cheap pre-checks; these are the race-safe ones.
      player.ownedCharacters = player.ownedCharacters ?? []
      if (player.ownedCharacters.includes(CHARACTER_ID)) {
        outcome = { reason: 'owned' }
        return
      }

      player.wallet = player.wallet ?? {}
      let gems = roundGems(player.wallet.gems ?? 0)
      const results = []
      let won = false
      let spinsUsed = 0

      for (let i = 0; i < spinCount; i++) {
        if (gems < COST_PER_SPIN) break
        if ((player[SPIN_FIELD] ?? 0) >= MAX_SPINS_PER_PLAYER) break

        const nextSpin = (player[SPIN_FIELD] ?? 0) + 1
        const chance = chanceForExclusiveSpin(nextSpin, {
          deadZoneUntil: DEAD_ZONE_UNTIL,
          plateauChance: PLATEAU_CHANCE,
          pityAt: PITY_AT,
        })
        const thisWon = nextSpin >= PITY_AT || Math.random() < chance

        gems = roundGems(gems - COST_PER_SPIN)
        player.wallet.gems = gems
        player[SPIN_FIELD] = nextSpin
        spinsUsed++
        results.push({ spin: nextSpin, won: thisWon })

        if (thisWon) {
          player.ownedCharacters.push(CHARACTER_ID)
          won = true
          break
        }
      }

      if (!results.length) {
        outcome = {
          reason: (player[SPIN_FIELD] ?? 0) >= MAX_SPINS_PER_PLAYER ? 'exhausted_lifetime' : 'gems',
          gems,
        }
        return
      }

      outcome = {
        reason: won ? 'won' : 'exhausted',
        results,
        spinsUsed,
        remaining: player.wallet.gems,
      }
    })

    if (outcome?.reason === 'owned') {
      return replyOverArt(ctx, character,
        `♾️ *${character.name} is already yours.*\n${RULE}\n` +
        `_Equip him with *${p}character equip ${CHARACTER_ID}*._`)
    }
    if (outcome?.reason === 'gems') {
      return ctx.reply(
        `💎 *Not enough Gems.*\n${RULE}\n` +
        `One spin costs 💎*${COST_PER_SPIN}*. You hold 💎*${fmtGems(outcome.gems)}*.\n\n` +
        `_Come back with more._`,
      )
    }
    if (outcome?.reason === 'exhausted_lifetime') {
      return ctx.reply(
        `♾️ *No spins left.*\n${RULE}\n` +
        `_You have used all *${MAX_SPINS_PER_PLAYER}* of your spins for ${character.name}. ` +
        `The distance never once closed._`,
      )
    }

    const last = outcome.results[outcome.results.length - 1]
    const spent = roundGems(outcome.spinsUsed * COST_PER_SPIN)
    const range = outcome.spinsUsed === 1
      ? `Spin *${last.spin}*`
      : `Spins *${outcome.results[0].spin}* → *${last.spin}*`

    if (outcome.reason === 'won') {
      const ability = character.ability
      return replyOverArt(ctx, character,
        `♾️🟦🟥 *THROUGHOUT HEAVEN AND EARTH* 🟥🟦♾️\n${RULE}\n` +
        `_Spin ${last.spin}. The Six Eyes open, Infinity settles over you like still water, ` +
        `and the strongest simply decides you are worth his time._\n\n` +

        `🏆 *${character.name.toUpperCase()} OBTAINED*\n` +
        `${character.emoji ?? '♾️'} *${characterStars(character.stars)}*` +
        (ability?.name ? `  ·  *${ability.name}*` : '') + `\n` +
        (ability?.flavor ? `_${ability.flavor}_\n` : '') +
        (character.description ? `\n_${character.description}_\n` : '') +

        `\n_Taken on spin ${last.spin}, for 💎${spent} this pull._\n` +
        `💎 Remaining: 💎*${fmtGems(outcome.remaining)}*\n\n` +
        `*How to use him:*\n` +
        `1️⃣ *${p}character equip ${CHARACTER_ID}*\n` +
        `2️⃣ *Infinity* needs no command. Blows shrink as they reach you, and the small ones never arrive at all\n` +
        `3️⃣ *${p}hollowpurple*, once per battle: Blue draws in, Red throws out, and the clash of the two erases what is between\n` +
        `4️⃣ *${p}unlimitedvoid*, once per battle: open the domain and drown the enemy in infinite information, so they cannot act\n`,
      )
    }

    const lines = [
      `♾️ *INFINITY HOLDS*`,
      RULE,
      `_You spent 💎${spent}. The distance never closed._`,
      ``,
      range,
      reel(outcome.results),
      ``,
      missLine(last.spin),
      ``,
      `📿 Pity  ${pityBar(last.spin)}  *${last.spin}*/${MAX_SPINS_PER_PLAYER}`,
      `💎 Remaining: 💎*${fmtGems(outcome.remaining)}*`,
    ]

    if (requested > MAX_SPINS_PER_COMMAND) {
      lines.push(``, `_Capped at *${MAX_SPINS_PER_COMMAND}* spins per command. Run it again for more._`)
    } else if (outcome.spinsUsed < spinCount) {
      lines.push(``, `_Gems ran out after *${outcome.spinsUsed}* of *${spinCount}* requested spins._`)
    }

    const lossText = lines.join('\n')
    // Art preview only on a player's very first ever spin attempt. Every losing
    // spin after that is text only, matching the other spins.
    return isFirstEverAttempt ? replyOverArt(ctx, character, lossText) : ctx.reply(lossText)
  },
}
