...
import { isTitled, ensurePrestige, getTierForXp, getPrestigeProgress } from './title-engine.js'
...
function drawRankBar(ctx, card, startY, view, colors) {
  ...
  ctx.textAlign = 'left'
  const availW = barW - 52 - pctW
  ctx.fillStyle = colors.white

  if (view.titleGlyph) {
    // Titled player: draw "<glyph> <tierName>" as two fillText calls in two
    // different fonts (see lib/title-glyphs.js). fitFont can't be reused
    // as-is here because it measures in a single family — shrinking against
    // the name alone would leave the (usually wider) glyph segment
    // unaccounted for and overflow the bar, and shrinking against a
    // concatenated string would measure the glyph in the wrong font
    // entirely. So this fits the combined width across both fonts by hand,
    // shrinking one size step at a time exactly like fitFont does.
    const weight = 'bold'
    let size = 22
    const fits = () => {
      const font = `${weight} ${size}px sans-serif`
      const glyphW = titleTextWidth(ctx, view.titleGlyph, font)
      ctx.font = font
      const nameW = ctx.measureText(view.rankTitle).width
      return glyphW + 8 + nameW <= availW
    }
    while (size > 13 && !fits()) size -= 1
    const font = `${weight} ${size}px sans-serif`
    drawBadgedName(ctx, x + 26, startY + barH / 2 + 8, {
      glyph: view.titleGlyph,
      name: view.rankTitle,
      font,
      gap: 8,
      color: colors.white,
    ...
    fitFont(ctx, view.rankTitle, availW, { size: 22, min: 13 })
...
function buildView(player, publicView) {
  ...
  // Level-200 prestige titles replace the ordinary Rank bar once a player
  // is titled (see lib/title-engine.js): data/ranks.json already tops out
  // at level 100 ("Shadow Sovereign", 91-100) with nothing above it, so
  // getRankForLevel() would otherwise just keep returning that same entry
  // forever from 101 all the way past 200 — titles give that endgame span
  // something that actually keeps moving. titleGlyph is drawn in the Noto
  // Sans Symbols font (see drawRankBar below); rankTitle stays plain text
  // for everyone else, drawn exactly as before.
  const titled = cap != null && isTitled(p, cap)
  const prestigeState = titled ? ensurePrestige(p) : null
  const prestigeProg = prestigeState ? getPrestigeProgress(prestigeState.xp) : null
  const tier = prestigeProg?.tier ?? null
  ...
  const rawName = String(p.name ?? 'Unknown').slice(0, 28) || 'Unknown'
  const tag = getGuildTag(p)
  const displayName = tag ? `${tag} ${rawName}` : rawName
  ...
  const view = {
    ...
    rankTitle: tier ? tier.name : rank.title,
    // Set only for a titled player. drawRankBar draws titleGlyph (in the
    // Noto Sans Symbols font) immediately before rankTitle (in the normal
    // font) as two separate fillText calls — see lib/title-glyphs.js's
    // header for why @napi-rs/canvas can't do this in one call.
    titleGlyph: tier ? tier.glyph : null,
    xpPct: titled ? prestigeProg.pct : prog.pct,
    // At Ⓛ🅜 (the top tier) there's nowhere further to climb, so the bar
    // reads MAX exactly like a non-reborn player pinned at their level
    // ceiling — prestigeProg.maxed is true only for that top tier, NOT for
    // every titled player (level 200 alone would otherwise always read MAX
    // here, since prog.maxed compares level to cap and titled players are
    // always AT cap by definition).
    xpMaxed: titled ? prestigeProg.maxed : prog.maxed,
    stats: [],
    side: null,
    bottomStats: null,
  }
  ...
  if (publicView) {
    ...
    // on the bar right below it. A titled player has moved past ranks
    // entirely (data/ranks.json tops out at level 100, so rank.epithet
    // would otherwise show the same "Beyond Rank" flavor text for every
    // level from 101 to 200+), so this box shows their tier name instead.
    view.stats = [
      { value: String(level), label: 'Level' },
      { value: tier ? tier.name : rank.epithet, label: tier ? 'Title' : 'Epithet' },
    ]
    return view
  }
