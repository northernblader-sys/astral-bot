...
import { applyVeteranScaling } from './combat-engine.js'
import {
  applyIncomingDamage,
  hasWitchOfEnvy,
...
import { tickShunShunRikka } from './orihime.js'
...
// Veteran (level 100+) difficulty, spent at PARTIAL weight on a pack.
//
// lib/combat-engine.js's veteran tier raises a monster's HP/ATK/DEF for players
// past the level the whole game was tuned for. In a swarm that has to be spent
// more carefully than on a boss: these weights apply a fraction of the excess,
// because a floor spawns two to four of them at once, so a full bump on each
// monster is four walls for the price of one and the floor stops being a fight
// you can read. HP leads (the fight gets longer and the telegraphs matter more),
// ATK follows modesty (they already swing from every lane), DEF barely moves.
const VETERAN_PACK_WEIGHTS = { hp: 0.5, atk: 0.6, def: 0.75 }
...
export function buildSwarmFloor(locationId, floor, player = null) {
  ...
  // Veteran tier on top of the ratio scaling — a no-op below level 100 up to
  // 200, so the whole ordinary climb is untouched (see applyVeteranScaling).
  if (player) for (const m of monsters) applyVeteranScaling(m, player, VETERAN_PACK_WEIGHTS)
...
async function processSwarmTurn(player, ctx, action) {
  ...
  // Orihime — Shun Shun Rikka ticks in the swarm too: heal + one rejection.
  ...
  // ── Satella's Wonders of You in the swarm. No other equipped-character
  // turn-start passive runs anywhere in this file, so this is her only hook
  // here. She treats the whole pack as one enemy: a halve clips every live
  // monster, and her final form wipes the floor outright. A monster has no
  // defend action to read, so the reprieve can never be earned in a swarm and
  // she always marches to the end. The floor-clearing wipe is handed off on the
  // exact same terms as a normal last-monster kill (see step 2 below).
  if (hasWitchOfEnvy(player)) {
    ...
    const envy = advanceWondersOfEnvy(bs, {
      ...
      immune: false,
      oppDefendedLast: false,
    })
    if (envy) {
      if (envy.art) {
        void sendImage(ctx, envy.art, `🖤 ${player.name}'s Witch of Envy takes her final form.`).catch(() => {})
      }
      if (envy.halveFraction) {
        for (const m of live) m.hp = Math.max(1, Math.floor(m.hp * envy.halveFraction))
      }
      if (envy.lines) msg += envy.lines + '\n'
      if (envy.forcedLoss) {
        if (envy.drainOwner) player.hp = 1
        for (const m of live) { m.hp = 0; m.alive = false }
        const target = live[live.length - 1] ?? bs.monsters[bs.monsters.length - 1]
    ...
    if (action.kind === 'attack' && !target) {
      ...
      if (Math.random() > calcPlayerHitChance(player, target)) {
        ...
        if (target.hp <= 0) {
          ...
          const paid = creditKill(player, target, ctx, {
            battleType: 'dungeon',
            killTurns: killTurnsFor(bs, target),
            locId: bs.locationId,
          })
          if (paid.boardMsg) msg += paid.boardMsg + '\n'
        }
      }
    }
