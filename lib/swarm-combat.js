 ...
 */
export function buildSwarmFloor(locationId, floor) {
...
export async function resolveSwarmAttack(player, ctx) {
  const bs = player.battleState
  ...
  if (/^\d+$/.test(arg)) {
    ...
    if (!target) {
      await ctx.reply(`❓ No target *#${arg}*. Live: ${list.map((m, i) => `${i + 1}. ${m.name}`).join(' · ')}`)
      return player
    }
  } else {
    target = nearestThreat(bs)
  }
  return processSwarmTurn(player, ctx, { kind: 'attack', target })
}
 ...
 *
 * Called from an ability plugin exactly like resolveSwarmAttack: inside the
...
// ── Frame ────────────────────────────────────────────────────────────────
/**
 * The text battlefield. Each live monster shows its number (matching
 * `.attack <n>`), lane/approach, a wind-up warning, and an HP bar; then the
 * player's lane and bars; then who strikes next turn and the controls.
 */
export function renderSwarmFrame(bs, player) {
  const p = config.prefix
  const list = displayList(bs)
  let out = `⚔️ *FLOOR ${bs.floor} — SWARM*${bs.isApprentice ? '  💀 _Apprentice Wave_' : ''}\n`
  out += `🧭 You hold the *${laneName(bs.playerLane)} lane*\n\n`
  list.forEach((m, i) => {
    const pos = m.range === 'far' ? '🔭 closing in' : `⚔️ ${laneName(m.lane)} lane`
    const warn = m.telegraph ? '  ⚠️ *winding up!*' : ''
    out += `*${i + 1}.* ${m.emoji ?? '👹'} ${m.name} _(${pos})_${warn}\n   ${hpBar(m.hp, m.maxHp)}\n`
  })
  out += `\n👤 *${player.name ?? 'You'}*  ${hpBar(player.hp, player.maxHp)}  💧 ${player.mp ?? 0}/${player.maxMp ?? 0}\n`
  const incoming = list.filter((m) => m.telegraph)
  out += incoming.length
    ? `\n🎯 *Next turn:* ${incoming.map((m) => m.name).join(', ')} will strike. Kill it, ${p}dodge, or move clear.\n`
    : `\n_The field is still... for now._\n`
  out += `\n▸ *${p}attack _[#]_* · *${p}skill* · *${p}ml* / *${p}mr* · *${p}dodge* · *${p}defend* · *${p}flee*`
  return out
}
