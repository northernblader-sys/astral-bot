...
import { applyStruckReactions, applyPackLifestealOnDeal } from './premium-abilities.js'
...
function resolveWagerHit(actor, opp, action, skill) {
  ...
  const armorWear  = wearArmorOnHit(opp, landed)
  const wearLines = [breakMessage(weaponWear), breakMessage(armorWear)].filter(Boolean).join('')

  // Premium ability passive + pack thorns/lifesteal. Both fighters are live
  // objects here, so this mirrors the PvE hit-sites directly: the attacker heals
  // from Dark Monarch's Dread, and a still-standing defender punishes them
  // (freeze/burn/sleep chance, Gemstone flame, Sicilian riposte). Riposte is
  // floored non-lethal to stay clear of the self-tick death settlement below.
  if (landed > 0) {
    const ls = applyPackLifestealOnDeal(actor, landed)
    if (ls.heal > 0) {
      actor.hp = Math.min(actor.maxHp, actor.hp + ls.heal)
      lines.push(...ls.lines)
    }
  }
  if (landed > 0 && opp.hp > 0) {
    const struck = applyStruckReactions(opp, actor, landed)
    if (struck.lines.length) lines.push(...struck.lines)
