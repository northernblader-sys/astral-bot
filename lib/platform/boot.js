...
import { updateAllPlayers, flushPendingWrites, installReadGuard } from '../player-repo.js'
...
export async function initDb(dbPath = config.dbPath) {
  ...
  // Same read guard the WhatsApp entry point installs — see
  // lib/player-repo.js's installReadGuard() for why every raw db.read() in a
  // plugin has to flush the debounced player write first or it rewinds state.
  installReadGuard(db)
  await db.read()
