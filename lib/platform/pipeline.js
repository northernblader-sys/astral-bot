...
const BATTLE_ALLOWED_COMMANDS = new Set([
  ...
  // Echidna's Gospel of Greed and Book of Wisdom (plugins/greed.js,
  // plugins/wisdom.js). `.greed` is her once-per-battle tithe and `.wisdom`
  // is her in-battle readout, so both are battle-only in a PvE fight, and
  // both were missing here from the day they were written: a Discord or
  // Telegram player's `.greed` was rejected by this gate before plugins/greed.js
  // ever ran. Every alias is listed because cmd is the raw typed token.
  // The `.echidna` hub command stays out on purpose, exactly as in handler.js:
  // its ritual/child/name subcommands are not battle actions.
