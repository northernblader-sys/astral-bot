...
/**
 * Effects that have been permanently removed from the game.
 * Players may still own mods with these IDs (bought or installed before
 * the removal), but they never activate — hasMod() returns false and
 * activateMod() blocks with a "disconnected" error so the player sees
 * exactly why it won't work rather than a silent no-op.
 */
export const DISCONNECTED_EFFECTS = new Set([
  'no_death_inventory_loss', // Undying Grace — retired: too forgiving on death
  'no_death_currency_loss',  // Coin Ward — retired: solar loss on death not implemented
  'no_durability_loss',      // Unbreaking Grip — retired: negates durability system
])
 ...
 *  Always returns false for disconnected effects — they are owned but inert. */
export function hasMod(player, effectId) {
  if (DISCONNECTED_EFFECTS.has(effectId)) return false
...
/**
 * True if the player OWNS a mod with this effect id even though the effect is
 * disconnected (bought/activated before the effect was retired). Used purely
 * for transparency messaging — e.g. handleDeath tells a player whose
 * Undying Grace just silently did nothing WHY it did nothing, instead of
 * leaving them to report "my keep-inventory cheat didn't work" as a bug.
 * Never grant effects off this: the effect itself stays disconnected.
 */
export function ownsDisconnectedMod(player, effectId) {
  if (!DISCONNECTED_EFFECTS.has(effectId)) return false
  return (player?.mods?.owned ?? []).some(m => m?.effectId === effectId)
}
...
export function activateMod(player, instanceId) {
  ...
  if (DISCONNECTED_EFFECTS.has(inst.effectId)) return { ok: false, error: 'disconnected' }
