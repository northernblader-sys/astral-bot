...
import { getPlayer, playerExists, createPlayer, updatePlayer } from './player-repo.js'
...
function issueSession(res, player) {
  ...
  // Cookie for same-site convenience; the frontend is on a different origin
  // (Vercel) where third-party cookies are increasingly blocked, so the
  // bearer token in the body is the primary mechanism and the cookie is the
...
function playerCore(db, p) {
  ...
  return {
    ...
    record: { wins: p.stats?.wins ?? 0, losses: p.stats?.losses ?? 0 },
...
const BOARDS = {
  ...
  wins: {
    ...
    unit: 'Wins',
    requirePositive: true,
    metric: p => p.stats?.wins ?? 0,
    tiebreak: p => -(p.stats?.losses ?? 0),
    format: p => `${p.stats?.wins ?? 0}W – ${p.stats?.losses ?? 0}L`,
  },
}
...
function buildApp(db, instances) {
  ...
  app.post('/api/auth/verify-otp', wrap(async (req, res) => {
    ...
    if (handle) {
      ...
      const token = issueSession(res, existing)
    ...
    if (!existing) {
      ...
      const token = issueSession(res, { id: jid })
...
export function startApiServer(db, instances) {
  ...
  const server = app.listen(config.apiPort, () => {
    log(`listening on :${config.apiPort} → ${config.publicApiUrl}`)
