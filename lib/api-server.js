...
import { POKE_ITEM_MAP, itemIconUrl as pokeItemIconUrl } from './pokegame-items.js'
...
function itemSummary(id) {
  const it = itemMap[id]
  // Pokémon items (poke_*) live in data/pokemon-items.json, not in itemMap, so they used to show up
  // as a bare id with no name or art. Resolve them here so /api/me lists them properly.
  if (!it && POKE_ITEM_MAP.has(id)) {
    const pi = POKE_ITEM_MAP.get(id)
    return {
      id, name: pi.name, emoji: null, rarity: pi.rarity ?? null, type: `pokemon-${pi.category}`,
      image: safeImageUrl(pokeItemIconUrl(id)), description: pi.description ?? null,
    }
  }
  return {
    id,
...
function characterSummary(id) {
  ...
  return {
    ...
    /* Owner freeze (`.lockspin`, lib/spin-locks.js). The site's banner payloads
       already carry this as `locked` — see spinBannerState() — but the character
       cards and /api/characters did not, so the storefront kept rendering a
       live 🪙 price for a character the shop now refuses to sell. Read it as
       "closed for everyone": it is the same flag chat gates both doors on. */
    frozen: isSpinLocked(c.id),
...
function buildApp(db, instances) {
  ...
  /* ── Pokémon game (Android app): /api/pokemon/* ── */
  // Pokémon Showdown is ~140 MB once loaded, so it is imported on the first request rather than at
  // boot: a bot nobody plays the game on pays nothing. If it fails to load, only these routes 503.
  let pokemonRouterPromise = null
  app.use('/api/pokemon', wrap(async (req, res, next) => {
    try {
      pokemonRouterPromise ??= (async () => {
        const { registerPokemonRoutes } = await import('./api-pokemon.js')
        const r = express.Router()
        registerPokemonRoutes(r, { db, requirePlayer, wrap, updatePlayer, log })
        return r
      })()
      const router = await pokemonRouterPromise
      return router(req, res, next)
    } catch (err) {
      pokemonRouterPromise = null
      log(' Pokémon game failed to load:', err?.stack ?? err?.message ?? err)
      return res.status(503).json({ ok: false, error: 'The Pokémon game is unavailable right now.' })
    }
  }))

  // Dungeon combat drives the real chat plugins (plugins/dungeon.js + the combat
  // verbs) through a capture-ctx — see lib/api-dungeon.js. Lazy-imported like the
  // Pokémon router so a boot never pays for the combat/render graph until the
  // first dungeon request, and any import failure isolates to a 503 here.
  let dungeonRouterPromise = null
  app.use('/api/dungeon', wrap(async (req, res, next) => {
    try {
      dungeonRouterPromise ??= (async () => {
        const { registerDungeonRoutes } = await import('./api-dungeon.js')
        const r = express.Router()
        registerDungeonRoutes(r, { db, requirePlayer, wrap, updatePlayer, log })
        return r
      })()
      const router = await dungeonRouterPromise
      return router(req, res, next)
    } catch (err) {
      dungeonRouterPromise = null
      log(' Dungeon combat failed to load:', err?.stack ?? err?.message ?? err)
      return res.status(503).json({ ok: false, error: 'Dungeon combat is unavailable right now.' })
    }
  }))
