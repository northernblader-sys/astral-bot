...
/**
 * Character star rating (1-5, or the beyond-the-scale 6) -> a plain star bar,
 * e.g. "★★★★☆". Everyone at five stars or below gets the classic five-slot
 * bar. Six is reserved for Sword Maiden, who stands above the entire roster,
 * and renders as a full six-star bar with no empty slots. Characters are
 * rated on their own explicit star scale (the `stars` field in
 * data/characters.json), independent of the item `rarity` ladder above.
 * A missing or out-of-range value is clamped into [0,6].
 */
export function characterStars(stars) {
  const n = Math.max(0, Math.min(6, Math.round(Number(stars) || 0)))
  if (n >= 6) return FILLED.repeat(6)
