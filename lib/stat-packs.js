...
function splitTotal(total, parts, rand) {
  ...
  const cuts = new Set()
  // Attempt-bounded on purpose. This loop needs parts-1 DISTINCT cut points, so
  // a `rand` that keeps returning the same value (a constant test stub, a seeded
  // PRNG wired up wrong, a mocked Math.random) can never fill the set — and the
  // unbounded version of this loop spun forever on it. That is not a harmless
  // hang: applyStatPack() is called with the gems already deducted inside the
  // player's updatePlayer lane, so the player pays, the lane never returns, and
  // the bot looks frozen for that player until the 20s task timeout.
  let attempts = 0
  while (cuts.size < parts - 1 && attempts++ < 200) {
    cuts.add(1 + Math.floor(rand() * (total - 1)))
  }

  if (cuts.size < parts - 1) {
    // Deterministic fallback: an even composition of exactly `total` into
    // `parts` positive integers. Same contract, no dependence on `rand`.
    const out = []
    let remaining = total
    for (let i = 0; i < parts - 1; i++) {
      const size = Math.max(1, Math.floor(remaining / (parts - i)))
      out.push(size)
      remaining -= size
    }
    out.push(remaining)
