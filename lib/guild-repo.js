...
export const GUILD_TAGS = {
  astral_vanguard: '[⚔️ VANGUARD]',
  shadow_covenant: '[🌑 SHADOW]',
  gilded_order:    '[⚜️ GILDED]',
  stormbreakers:   '[⛈️ STORM]',
  emberwake:       '[🔥 EMBER]',
}

/** Returns the official badge/tag text for a player's guild, or empty string if guildless. */
export function getGuildTag(guildIdOrPlayer) {
  const gid = typeof guildIdOrPlayer === 'string' ? guildIdOrPlayer : guildIdOrPlayer?.guildId
  if (!gid) return ''
  return GUILD_TAGS[gid] ?? ''
}
