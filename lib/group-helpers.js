...
/**
 * ── Warm-ahead (2026-09: command reply latency) ────────────────────────────
 *
 * A cache miss here is a NETWORK round trip in front of the message handler:
 * runModerationScans awaits checkBotAdmin() before dispatch, so the first
 * message after each group's TTL expired paid the round trip (typically
 * 100-800ms, sometimes seconds when WhatsApp is slow) before the command even
 * started — and with many groups in use, someone was always paying it. Those
 * are exactly the "the bot takes forever sometimes" reports.
 *
 * So a message that hits a nearly-expired entry now kicks off the SAME refresh
 * in the background and answers from the copy it has. The fetch rate is
 * unchanged (the refresh fires ~8s before expiry, i.e. still about once per
 * TTL window per group — deliberately NOT more, because groupMetadata() is
 * itself rate limited by WhatsApp), and old age is still handled exactly as
 * before: a caller that arrives after the entry expired with no refresh
 * running will await a fresh fetch, and a failed fetch still keeps the
 * existing backoff + stale-on-failure behaviour.
 */
const GROUP_META_WARM_AFTER_MS = GROUP_META_TTL_MS - 8_000

const groupMetaCache = new Map()
const groupMetaFailures = new Map()
/** groupId → in-flight background refresh, so a burst of messages starts one. */
const groupMetaRefreshes = new Map()

/**
 * Refresh this group's metadata without blocking the caller.
 *
 * No-op while a refresh is already in flight, while the entry is still young
 * enough to serve, and during the failure backoff window.
 */
function warmGroupMetadata(sock, groupId) {
  if (typeof sock?.groupMetadata !== 'function') return null

  const inFlight = groupMetaRefreshes.get(groupId)
  if (inFlight) return inFlight

  const entry = groupMetaCache.get(groupId)
  if (entry && Date.now() - entry.at < GROUP_META_WARM_AFTER_MS) return null

  const failed = groupMetaFailures.get(groupId)
  if (failed && Date.now() - failed.at < GROUP_META_FAIL_BACKOFF_MS) return null

  let promise
  try {
    promise = Promise.resolve(sock.groupMetadata(groupId))
      .then(meta => {
      ...
      .catch(err => {
        // Same bookkeeping as a foreground fetch: the next caller falls back
        // to the stale copy, and the backoff keeps us off a rate-limited API.
        groupMetaFailures.set(groupId, { at: Date.now(), err })
        return null
      })
      .finally(() => { if (groupMetaRefreshes.get(groupId) === promise) groupMetaRefreshes.delete(groupId) })
  } catch (err) {
    // sock.groupMetadata() can throw synchronously on a socket that is closing.
    groupMetaFailures.set(groupId, { at: Date.now(), err })
    return null
  }

  groupMetaRefreshes.set(groupId, promise)
  return promise
}
...
export async function getGroupMetadata(sock, groupId, { maxAgeMs = GROUP_META_TTL_MS, allowStale = true } = {}) {
  ...
  if (hit && Date.now() - hit.at < maxAgeMs) {
    // Answer immediately; keep the entry from going cold on a busy group so
    // the next message doesn't have to wait on the network (see warm-ahead).
    if (maxAgeMs >= GROUP_META_TTL_MS) warmGroupMetadata(sock, groupId)
    return hit.meta
  }
