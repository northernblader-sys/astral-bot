...
import { PRESTIGE_TIERS } from './title-engine.js'
...
/**
 * formatFullRankLadder() — every rank a player can achieve, HIGHEST first
 * down to Beginner (the `.ranks` command, plugins/ranks.js):
 *   1. the post-level-200 prestige titles (LM → GM → Am → Pro) from
 *      lib/title-engine.js — the "new" titles that were previously invisible
 *      in the rank table, shown here as the top of the ladder;
 *   2. the ordinary Lv 1–100 ranks, Shadow Sovereign down to E-Rank Hunter
 *      (epithet "Newbie" — the beginner rung).
 */
export function formatFullRankLadder() {
  const lines = []
  for (const tier of [...PRESTIGE_TIERS].reverse()) {
    const xp = tier.xpRequired === 0
      ? '_instant at max level_'
      : `_${tier.xpRequired.toLocaleString()} prestige XP_`
    lines.push(`${tier.glyph} *${tier.name}* — post-Lv 200 · ${xp}`)
  }
  for (const r of [...ranks].reverse()) {
    const beginner = r.min === 1 ? ` _(Beginner)_` : ''
    lines.push(`${r.emoji} *${r.title}* _(${r.epithet})_${beginner} — Lv ${r.min}–${r.max}`)
  }
  return lines.join('\n')
}
 ...
 *
 * `capOverride` exists for reborn players, whose personal ceiling is 200
