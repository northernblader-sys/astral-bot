 ...
 *
 * Every command used to cost at least TWO sends: handler.js auto-reacted ⚔️
 * to the message, then the plugin replied (the per-command auto-react was
 * removed 2026-09 — see handler.js's note — which removed half of this
 * traffic outright). With maxPerMinute at 40 that was a
 ...
 *
 * Reported symptom: "when the group overspam, the bot briefly loses its
 * WhatsApp connection." Reactions were the amplifier: every command used to
 * fire an auto-react (handler.js — removed 2026-09), and the react lane was
 * deliberately UNPACED (invisible to the pacer, see the note in drain()
 * below): a person typing `.attack` ten times in a burst produced ten react
 * stanzas on top of the ten queued replies, and the reacts all landed inside
 * the same second. WhatsApp's spam heuristics read that burst as bot
 * behaviour and answer with rate-overlimit, and at the server's limit a
 * device gets dropped for a beat before it is allowed back: exactly the
 * brief disconnect being reported. The lane still exists for the reactions
 * the plugins fire on their OWN results (downloader ✅/❌, gif2mp4, manga).
 *
