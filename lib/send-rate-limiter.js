...
export function wrapSendWithRateLimit(sock, { botName, minGapMs = 2_500, maxPerMinute = 20, log = () => {} } = {}) {
  ...
  const stats = { dropStale: 0, dropOverflow: 0, dropReact: 0, timeouts: 0, failed: 0, sent: 0 }
  let lastBacklogWarnAt = 0

  function nextGapMs() {
    // +/- 20% jitter around minGapMs so sends don't land on a perfectly
    // even beat, which is itself a signal spam filters key on.
    const jitter = minGapMs * 0.2
    return minGapMs + (Math.random() * jitter * 2 - jitter)
  }
    ...
    try {
      while (queue.length || lowQueue.length) {
        ...
        try {
          ...
          stats.failed++
          log(`⚠️ [${botName}] sendMessage failed (queued ${new Date(job.queuedAt).toISOString()}): ${err.message}`)
          job.reject(err)
        }
      }
    } finally {
  ...
  sock.sendMessage = function rateLimitedSendMessage(...args) {
    return new Promise((resolve, reject) => {
      ...
      if (isLowPriority(args)) {
        ...
        const now = Date.now()
        if (queue.length > maxPerMinute && now - lastBacklogWarnAt > 60_000) {
          ...
          log(
            ...
            `${sentTimestamps.length}/${maxPerMinute} sent this minute. ` +
