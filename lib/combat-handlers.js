...
import { cleanupBossFight, buildEnemyAttack, applyBossSpecial, EVENT } from './boss-engine.js'
import { speedKillXp, DUNGEON_KILL_XP } from './xp-regulator.js'
...
import { hasMod, getModValue } from './mods.js'
...
export function creditKill(player, enemy, ctx, { battleType, killTurns, locId }) {
  ...
  // The flat 2x and the 100 XP floor are legacy grind-rate levers that only
  // apply OFF the speed band. On the band they'd both defeat it — the 2x would
  // make it 60–120 and the floor would flatten every result to a constant 100,
  // erasing the speed difference entirely.
  const xpMult  = onSpeedBand ? 1 : combatXpMultiplier
  const xpFloor = onSpeedBand ? 1 : 100
  const xp = Math.max(xpFloor, Math.floor(xpBase * xpMult * premiumMultiplier * xpCheatMultiplier * (1 + seasonXpPct) * dungeonBoostMult))
  let solars = Math.floor((enemy.solars ?? enemy.rewards?.solars ?? 0) * premiumMultiplier * solarsCheatMultiplier * (1 + seasonSolarsPct) * dungeonBoostMult)
...
export async function handleVictory(player, enemy, ctx) {
  ...
  if (enemy.isBoss && locId) {
    ...
    if (locId === 'season_01_ruins' && season) {
      ...
      player.seasonProgress.currentFloor = 51
  ...
  const defeatLine     = animeDef?.defeatLines?.length
    ? animeDef.defeatLines[Math.floor(Math.random() * animeDef.defeatLines.length)]
    : null
  const bossDefeatMsg = defeatLine ? `\n\n_${defeatLine}_` : ''
  ...
  await ctx.reply(
    ...
    dropMsg +
    (lvlMsgs.length ? `\n\n${lvlMsgs.join('\n')}` : '') +
    skillMsg + fameMsg + seasonMsg + conquestMsg + endFinaleMsg +
    (beastCpMsg ? `\n\n${beastCpMsg}` : '') +
    `\n\n❤️ HP: ${hpBar(player.hp, player.maxHp)}` +
    nextLine,
  )
...
export function processStatusTurn(entity) {
  const incapacitated = hasEffect(entity, 'stun') || hasEffect(entity, 'freeze')
