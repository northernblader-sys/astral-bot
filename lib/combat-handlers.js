...
import { sendRankUp } from './rank-up.js'
...
import { speedKillXp, DUNGEON_KILL_XP, LOCATION_REWARD_MULT, newcomerXpMult, NEWCOMER_BOOST } from './xp-regulator.js'
import { releaseDungeonSlot } from './dungeon-slots.js'
...
import { noteKill, readyLine } from './guild-board.js'
import { orihimeVictoryHeal } from './orihime.js'
import {
  resolveRescueVictory, resolveRescueDefeat, companionVictoryHeal, yeniseiIntercept,
} from './guardian-event.js'
import { sendCompanionOffer } from './guardian-ui.js'
import {
  grantYoriichiExp,
...
export async function resolveCatFormDefeat(player, ctx, { boss = false } = {}) {
  if (boss) cleanupBossFight(player)
  if (player.battleState?.type === 'rescue') return handleRescueLoss(player, ctx, '')
 ...
 */
export function checkPearlSave(player, ctx = null) {
  ...
  if (ctx) releaseDungeonSlot(ctx.isGroup ? ctx.sender : null, player, ctx.from); else releaseDungeonSlot(null, player, null)
...
export async function resolvePlayerHpZero(player, ctx, msg, { boss = false } = {}) {
  ...
  // Yenisei (Guardian of the Innocent companion) at trust 50: once per battle
  // she steps in front of the killing blow. Keeps the fight going, like the
  // totem above, so it sits ahead of anything that would end it.
  const yeniseiMsg = yeniseiIntercept(player)
  if (yeniseiMsg) return { fallThrough: true, msg: msg + yeniseiMsg }
  // A lost rescue battle is never a real death: no gear strip, no inventory
  // wipe, no pearl spent. The captives are dragged away and the player wakes
  // outside the pens. Checked before the pearl so the pearl is not wasted.
  if (player.battleState?.type === 'rescue') {
    return { fallThrough: false, returnValue: await handleRescueLoss(player, ctx, msg) }
  }
  const pearlMsg = checkPearlSave(player, ctx)
  if (pearlMsg) {
    if (boss) cleanupBossFight(player)
...
export function creditKill(player, enemy, ctx, { battleType, killTurns, locId }) {
  ...
  //
  // `locId` goes in because a location that tunes its monsters to a different
  // clear time re-anchors the band too (LOCATION_KILL_XP_BAND): caged_dimension
  // costs ~22 attacks per monster by design, so the global anchors would pay
  // every kill there the floor rate and undo the zone's 10x XP headline.
  const onSpeedBand = battleType === 'dungeon' && !enemy.isBoss
  const xpBase = onSpeedBand
    ? speedKillXp(killTurns, locId)
  ...
  // Newcomer boost (lib/xp-regulator.js) — a standing 1.5x on every kill while
  // the player is still in the early game, stacking with everything else here.
  // It is the smaller, everywhere-else version of the Newcomer's Hollow lane: it
  // makes questing, .work and wild kills worth something to someone who has not
  // found a dungeon group yet. 1 for anyone past untilLevel, so no veteran is
  // ever stronger for having it.
  const newcomerMult = newcomerXpMult(player)
  const xp = Math.max(xpFloor, Math.floor(xpBase * xpMult * premiumMultiplier * xpCheatMultiplier * homeXpMultiplier * (1 + seasonXpPct) * dungeonBoostMult * locationMult.xp * newcomerMult))
  ...
  // Guild-board culls. Same hook as the daily kill quest, so solo kills and
  // every swarm kill (including the floor-clearing one, which arrives here
  // through handleVictory) count. Party kills do not pass through creditKill;
  // plugins/party.js records those itself.
  const boardKill = noteKill(player, {
    name: enemy?.name,
    locationId: locId,
    isBoss: !!enemy?.isBoss,
  })
  const boardMsg = boardKill.ready ? readyLine(config.prefix) : ''
  ...
  return {
    xp, solars, xpBase, onSpeedBand, season, newcomerMult,
    beastCpMsg, dropMsg, lvlMsgs, rankChange, skillMsg, fameMsg, seasonMsg,
    titleMsg, prestigeResult, boardMsg,
  }
}
...
export async function handleVictory(player, enemy, ctx) {
  ...
  const guardianBs = player.battleState?.guardian ?? null
  ...
  // ── Guardian of the Innocent — a won rescue battle (plugins/rescue.js) ──
  // Pays the normal kill rewards through creditKill, then the rescue itself
  // (captives freed, rescue fame, milestones, the captor's companion request).
  // Never touches dungeon floors.
  if (battleType === 'rescue' && guardianBs) {
    return handleRescueWin(player, enemy, ctx, { guardianBs, killTurns })
  }
  ...
  await ctx.reply(
    ...
    (newcomerMult > 1 ? ` _(🌟 newcomer boost x${newcomerMult} — under level ${NEWCOMER_BOOST.untilLevel})_` : '') +
    speedLine +
    dropMsg +
    (lvlMsgs.length ? `\n\n${lvlMsgs.join('\n')}` : '') +
    ...
    (boardMsg ? `\n${boardMsg}` : '') +
    (beastCpMsg ? `\n\n${beastCpMsg}` : '') +
    orihimeVictoryHeal(player) + companionVictoryHeal(player) +
    `\n\n❤️ HP: ${hpBar(player.hp, player.maxHp)}` +
    nextLine,
  )

  // Rank promotion — separate message so it stands out. The dedicated
  // rank-up card comes from lib/rank-up.js, shared by every rank-up site.
  if (rankChange) {
    await sendRankUp(ctx, player.name, rankChange.from, rankChange.to)
  }
...
/**
 * handleRescueWin — the victory tail for a Guardian of the Innocent rescue
 * battle. battleState is already nulled by handleVictory; guardianBs is the
 * rescue payload it captured first.
 */
async function handleRescueWin(player, enemy, ctx, { guardianBs, killTurns }) {
  const p = config.prefix
  const {
    xp, solars, newcomerMult, beastCpMsg, dropMsg, lvlMsgs, rankChange, skillMsg, fameMsg,
    titleMsg, boardMsg,
  } = creditKill(player, enemy, ctx, { battleType: 'rescue', killTurns, locId: null })

  const result = resolveRescueVictory(ctx.db, player, guardianBs, {
    solarsPaid: solars,
    inventoryHasRoom: (pl) => hasInventoryRoom(pl, 1),
  })
  const heal = orihimeVictoryHeal(player) + companionVictoryHeal(player)

  await ctx.reply(
    `✅ *RESCUE WON!*\n\n${enemy.emoji ?? '⛓️'} *${enemy.name}* is down.\n\n` +
    `✨ *+${xp} XP*  ☀️ *+${solars} Solars*` +
    (newcomerMult > 1 ? ` _(🌟 newcomer boost x${newcomerMult})_` : '') +
    dropMsg +
    (lvlMsgs.length ? `\n\n${lvlMsgs.join('\n')}` : '') +
    skillMsg + fameMsg + titleMsg +
    (boardMsg ? `\n${boardMsg}` : '') +
    (beastCpMsg ? `\n\n${beastCpMsg}` : '') +
    `\n\n${result.text}` +
    heal +
    `\n\n❤️ HP: ${hpBar(player.hp, player.maxHp)}` +
    (result.offer ? '' : `\n_Type *${p}rescue* to free the next pen._`),
  )

  if (result.offer) await sendCompanionOffer(ctx, result.offer).catch(() => {})
  if (rankChange) await sendRankUp(ctx, player.name, rankChange.from, rankChange.to)
  return player
}

/** A lost rescue battle: no death penalty, the captives are dragged off. */
async function handleRescueLoss(player, ctx, msg = '') {
  const guardianBs = player.battleState?.guardian ?? null
  clearHypnosis(player)
  cleanupBossFight(player)
  player.inBattle = false
  player.battleState = null
  player.activeEffects = []
  player.hp = Math.max(1, Math.floor((player.maxHp ?? 1) * 0.1))
  const text = guardianBs ? resolveRescueDefeat(ctx.db, player, guardianBs) : '💔 *The rescue failed.*'
  player.battleRecord = player.battleRecord ?? { wins: 0, losses: 0 }
  player.battleRecord.losses = (player.battleRecord.losses ?? 0) + 1
  await ctx.reply(
    (msg ? `${msg.trimEnd()}\n\n` : '') + text +
    `\n\n❤️ HP: ${hpBar(player.hp, player.maxHp)}\n_Rest at the *${config.prefix}inn*, then try again._`,
  )
  return player
}
