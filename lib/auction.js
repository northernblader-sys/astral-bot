...
async function placeBid(ctx) {
  ...
  if (prevBidderId && prevBidderId !== player.id) {
    ...
    // DM the outbid player so they know their solars are back
    ctx.sock.sendMessage(prevBidderId, {
      text:
        `🏛️ *You were outbid on ${lot.item.name}!*\n\n` +
        `*${player.name}* bid *${amount.toLocaleString()} ☀️* — your *${prevBid.toLocaleString()} ☀️* has been refunded.\n\n` +
        `_Re-bid with *${config.prefix}auction bid ${lotNum} <higher amount>* if you still want it._`,
    }).catch(() => {})
  }
