...
const INSTANT_HANDLERS = {
  ...
  cure(entity, effectDef) {
    ...
    const NEGATIVE_TYPES = ['burn', 'poison', 'freeze', 'stun', 'sleep', 'blind', 'weaken', 'frostlock']
...
const DURATION_INITIALIZERS = {
  ...
  /**
   * sleep — hard CC; entity cannot act while active (Night Eyes). Behaves like
   * freeze for lockout purposes (see the incapacitated check in
   * lib/combat-handlers.js), kept a distinct type so narration reads "asleep"
   * and a wake-on-hit rule could be added later without touching freeze.
   * effectDef: { type: 'sleep', duration: number }
   */
  sleep(entity, effectDef) {
    entity.activeEffects = entity.activeEffects ?? []
    entity.activeEffects.push({
      type:      'sleep',
      remaining: effectDef.duration,
...
const TICK_PROCESSORS = {
  ...
  sleep(_entity, _entry) {
    return `😴 Asleep: cannot act.`
  },
...
export const NEGATIVE_EFFECT_TYPES = new Set([
  'burn', 'poison', 'freeze', 'stun', 'stun_chance', 'sleep',
...
export function absorbDamage(entity, incomingDamage) {
  ...
  // Pack signatures that act purely on the defender live here, at the one
  // funnel every struck hit routes through, so they cover PvE, PvP and wager
  // without any call site knowing about them. `activePackSignature` is a
  // snapshot written by `.pack equip` (plugins/pack.js) — a persistent equip,
  // not an activeEffects entry, so it survives pvpConclude's effect wipe the
  // same way player.permanentSever does.
  const sig = entity.activePackSignature
  if (sig && remaining > 0) {
    // Dodge (Arlnord's Divine Armor) / Phase (Space Sifter): a clean miss —
    // no damage taken, no shield/ironskin charges spent. Hard-capped so a
    // "dodge everything" pack can never be literally unhittable.
    if (sig.type === 'dodge' || sig.type === 'phase') {
      const chance = Math.max(0, Math.min(0.95, sig.chance ?? 0))
      if (Math.random() < chance) return 0
    }
    // Dread Sovereign (Dark Monarch): flat percentage cut off the top of every
    // hit, stacking before ironskin/shield below.
    if (sig.type === 'dread' && sig.incomingCut) {
      const cut = Math.max(0, Math.min(0.9, sig.incomingCut))
      remaining = Math.max(1, Math.floor(remaining * (1 - cut)))
    }
  }
...
export function processStatusTurn(entity) {
  ...
  const slept   = hasEffect(entity, 'sleep')

  let msg = null
  if (lines.length) msg = lines.join('\n')
  if (stunned && !msg) msg = `💫 Stunned — cannot act this turn.`
  if (frozen  && !msg) msg = `❄️ Frozen — cannot act this turn.`
  if (slept   && !msg) msg = `😴 Asleep — cannot act this turn.`

  return { stunned, frozen, slept, msg }
}
