 ...
 *
 * The monthly one-of-one gift (grantMonthlyExclusiveAbility) is the only
 * stateful piece: it claims through the shared exclusive-spin registry so a
 * given ability is held by exactly one player at a time — and
 * stripPremiumAbilities() releases that claim the moment Premium lapses.
 */
import {
  addStatusEffect, getEffectiveStat, hasEffect,
} from './effects.js'
import { premiumAbilityMap, premiumAbilities } from './game-data.js'
import { getExclusiveSpinWinner, claimExclusiveSpinForPlayer, releaseExclusiveSpinLock } from './season-engine.js'
...
export function applyAbilityOnStruck(owner, attacker) {
  ...
  switch (def.type) {
    ...
    case 'drowse_attacker':
      ...
      break
    case 'jack':
      // Weak versions of everything, rolled independently — and the draw really
      // has to be independent per branch. This used to be an else-if chain, so
      // ONE draw served all three checks: freeze is 10%, and if that draw is
      // high enough to miss freeze it is by definition also too high to be under
      // burn (10%) or sleep (8%). Measured over 200k hits the shipped chain
      // produced ~19.8k freezes, 0 burns and 0 sleeps — two thirds of the
      // ability, the fire and the sleep in its own flavour text, could never
      // fire at all. Each branch now draws its own number; the else-if is kept
      // so at most one proc lands per hit and the log stays one line.
      const rollFreeze = Math.random()
      const rollBurn   = Math.random()
      const rollSleep  = Math.random()
      if (rollFreeze < (def.freezeChance ?? 0)) tryFreeze(def.procTurns ?? 1)
      else if (rollBurn < (def.burnChance ?? 0)) tryBurn(def.dotPercent ?? 0.03, def.procTurns ?? 1)
      else if (rollSleep < (def.sleepChance ?? 0)) trySleep(def.procTurns ?? 1)
      break
    default:
      break
  }
...
// ── Standard premium ability (the floor, never an empty-handed buyer) ─────
//
// The product rule (kept from the original bug fix): NOBODY who pays for
// Premium leaves with an empty ability slot. The MONTHLY plan's hook is the
// guaranteed one-of-one gift below; this is the floor under everyone else —
// weekly/yearly buyers, renewals that already hold a one-of-one, and the rare
// monthly buyer who arrives while all five are held by active Premium holders.
//
...
// one-of-one: the 5 gifted abilities stay the exclusive legendaries.
export const PREMIUM_ABILITY_ID = 'premium_favor'

/**
 * grantPremiumAbility(player, giftResult) ->
 *   { granted: 'gift'|'new'|'already', id, equipped }
 *
 ...
 * with player.premium (same rule as grantMonthlyExclusiveAbility).
 *
 *  - 'gift'     — the buyer holds a one-of-one (just gifted, or already held
 *                 on a renewal): that IS their equipped ability (rendered
 *                 from player.premiumAbility), no standard grant on top.
 ...
 */
export function grantPremiumAbility(player, giftResult) {
  if (giftResult?.outcome === 'won' || giftResult?.outcome === 'already') {
    return { granted: 'gift', id: giftResult.abilityId ?? player.premiumAbility, equipped: true }
  }
...
// ── Monthly one-of-one gift ─────────────────────────────────────────────────
//
// This is NOT a spin and never rolls any dice: Premium is not a gamble for an
// ability. A monthly buyer is GIFTED one of the 5 one-of-ones outright —
// picked at random from whatever is still unclaimed, and if the player already
// holds one (an early renewal) it simply stays theirs. The old weekly
// luck-roll ('no_win' walking away empty) is gone.
...
/**
 * grantMonthlyExclusiveAbility(db, playerId, player)
 *   -> { outcome, abilityId?, ability? }
 *
 * MUST be called INSIDE an updatePlayer(db, playerId, ...) mutator so the
 * registry claim (claimExclusiveSpinForPlayer) is persisted atomically with
 * the player mutation — the loser of a race pays nothing and wins nothing.
 *
 * Outcomes:
 *   'already'  — the player already holds a one-of-one (renewal); it stays.
 *   'sold_out' — all 5 are currently held by other active Premium holders;
 *                the caller falls back to Crown's Favor so the buyer still
 *                leaves with an ability. (Stripping at expiry frees them
 *                again — see stripPremiumAbilities.)
 *   'won'      — abilityId/ability set on the player and claimed in the
 *                registry. Guaranteed whenever anything is unclaimed: there
 *                is no luck roll anywhere in this path.
 */
export function grantMonthlyExclusiveAbility(db, playerId, player) {
  ...
  for (const id of shuffled(unclaimed)) {
    if (claimExclusiveSpinForPlayer(db, abilityRegistryKey(id), playerId)) {
      ...
      return { outcome: 'won', abilityId: id, ability: premiumAbilityMap[id] }
    }
  }
  // Every remaining ability was claimed by a concurrent write between the
  // pre-check and here — fall back to the caller's standard grant.
  return { outcome: 'sold_out' }
}

// ── Expiry / revoke: everything Premium-granted leaves with the plan ────────
//
// An ability granted with Premium is a PERK of Premium, not a keepsake: when
// the plan lapses (the sweep in main.js) or is revoked (.premium-revoke), the
// one-of-one goes back on the shelf (registry claim released, so the next
// buyer can be gifted it) and Crown's Favor is pulled from inventory/equips.

/**
 * stripPremiumAbilities(player, db) -> { removedOneOfOne, removedCrown }
 *
 * MUST run INSIDE an updatePlayer/updateAllPlayers mutator, same as the
 * claim/release helpers it works through. Safe to call on a player with
 * neither grant (returns both falsy, writes nothing).
 */
export function stripPremiumAbilities(player, db) {
  let removedOneOfOne = null
  if (player.premiumAbility) {
    const key = abilityRegistryKey(player.premiumAbility)
    // Release the bot-wide claim only if this player actually holds it —
    // releaseExclusiveSpinLock frees whatever is there, and a mis-aimed strip
    // must never take another player's claim.
    if (getExclusiveSpinWinner(db, key) === player.id) {
      releaseExclusiveSpinLock(db, key)
    }
    removedOneOfOne = player.premiumAbility
    player.premiumAbility = null
  }

  let removedCrown = false
  const inv = player.abilityInventory ?? []
  const invKept = inv.filter((id) => id !== PREMIUM_ABILITY_ID)
  if (invKept.length !== inv.length) removedCrown = true
  player.abilityInventory = invKept

  const eq = player.equippedAbilities ?? []
  const eqKept = eq.filter((id) => id !== PREMIUM_ABILITY_ID)
  if (eqKept.length !== eq.length) removedCrown = true
  player.equippedAbilities = eqKept

  return { removedOneOfOne, removedCrown }
}
