...
/**
 * NotoSansSymbols-Regular.ttf — covers the Enclosed Alphanumeric Supplement
 * block (U+1F100–U+1F1FF), which none of the DejaVu faces above do. This is
 * the only font in the repo that can draw the level-200 prestige title
 * glyphs (Ⓟⓡⓞ / Ⓐ🅜 / Ⓖ🅜 / Ⓛ🅜 — see lib/title-engine.js) on a canvas card.
 * Registered under its own family name rather than folded into the generic
 * ALIASES table above: it should never win a plain 'sans-serif' request for
 * ordinary text (it only has symbol glyphs, so ordinary text would come out
 * blank), it should only be reached for deliberately by name. See
 * lib/title-glyphs.js for the helper that draws titles with it correctly.
 */
...
export function registerFonts() {
  ...
  register(FONT_DIR, SYMBOL_ALIASES)
