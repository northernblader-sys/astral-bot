...
async function callOnce(model, apiKey, payload, fetchImpl, timeoutMs) {
  ...
  try {
    ...
    try { json = await res.json() } catch (err) {
      ...
      if (res.ok) throw Object.assign(new Error('OpenRouter returned invalid JSON'), { status: 502, apiMessage: 'invalid_json' })
    }
    ...
    throw Object.assign(new Error(controller.signal.aborted ? 'OpenRouter request timed out' : 'OpenRouter network request failed'), {
...
export async function askAI({
  ...
  for (const candidate of ladder) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const text = await callOnce(candidate, key, payload, fetchImpl, timeoutMs)
