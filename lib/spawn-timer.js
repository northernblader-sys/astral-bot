...
import { fetchSpawnCard, tierStars, cardSellPrice, hasCardSeries } from './card-engine.js'
import { sendCardMedia } from './card-media.js'
import {
  fetchRandomSeries,
  getSeriesTier,
...
async function cardTick() {
  ...
  for (const groupJid of groupJids) {
    ...
    try {
      ...
      // GIF-aware send (lib/card-media.js) — same shape as main.js's sweep:
      // gif cards loop with gifPlayback, stills go as images, caption falls
      // back to text so the claim code is never lost to a dead image host.
      ...
      if (!sent) logger.warn({ jid: groupJid }, 'spawn-timer: card send failed (media + text fallback both failed)')
    } catch (err) {
