 ...
 * old gemPrice branch it used to have is gone. handleBuy() is also the single
 * place the owner freeze (`.lockspin`, lib/spin-locks.js) is checked against a
 * purchase, so a frozen character is unbuyable at any price.
 *
 ...
 *   exclusive  one-of-one bot-wide. Buyable while the CLAIM is free, and the
 *              buyer takes the claim with it, so a Mond purchase is a way to
 *              win the race rather than a way around it. Once claimed, no
 *              amount of Monds gets it (the check lives in handleBuy, since
 *              only it has the db handle to read the claim).
 *   spinOnly   not one-of-one (Yato, Gojo). Any number of buyers.
 *
 * Two DIFFERENT locks gate a purchase, and only one of them is price data:
 * the exclusive claim above (who owns the one copy) and the owner freeze in
 * lib/spin-locks.js (`.lockspin <name>`, whether the character is obtainable at
 * all). `isMondBuyable` answers the first from data alone; the second is read
 * by handleBuy at the moment of sale, because it is live state, not a field —
 * which is why `.character buy` on a frozen character is refused even though
 * this function still says it has a price.
 *
