...
export async function runPremiumActive(ctx, abilityId) {
  ...
  if (player.premiumAbility !== abilityId) {
    // Copy has to match how these are actually handed out: they are GIFTED with
    // the MONTHLY plan (lib/premium-abilities.js grantMonthlyExclusiveAbility),
    // never won on a spin. The old line sent a refused player off to buy weekly,
    // which does not carry a one-of-one at all.
    return ctx.reply(
      `🔒 You don't hold ${tag}. It's one of only *5* abilities in the game, each held by one player at a time, ` +
      `and it comes with the *monthly* Premium plan (*${pr}premium buy monthly*).`,
    )
  }
