...
/**
 * Resolve a typed username / character name to the account it stands for,
 * exactly the way /api/auth/lookup does (case-insensitive over username|handle|
 * name, first match wins). Returns the raw player record for SERVER-SIDE use
 * only — the caller must never put its JID (p.id, a phone number) on the wire.
 * Use targetHandle(p) for anything the app sees.
 */
function findPlayerByName(db, typed) {
  const target = String(typed ?? '').trim().replace(/^@+/, '').toLowerCase()
  if (target.length < 2) return null
  for (const p of Object.values(db.data.users ?? {})) {
    if (!p) continue
    const uname = String(p.username ?? p.handle ?? '').toLowerCase()
    const pname = String(p.name ?? '').toLowerCase()
    if (uname === target || pname === target) return p
  }
  return null
}

/** The safe, JID-free identity block for a resolved target player. */
function targetHandle(p) {
  return { name: p?.name ?? null, username: p?.username ?? p?.handle ?? null, level: p?.level ?? 1 }
}
...
export function registerPokemonRoutes(router, { db, requirePlayer, wrap, updatePlayer, log = console.log }) {
  ...
  /* ---- rename / protect (spec-shaped, over the same fields PATCH /mons/:id sets) ---- */
  router.post([`${R}/mons/:id/rename`, `${R}/:id/rename`], requirePlayer, wrap(async (req, res) => {
    const body = req.body ?? {}
    // Accept `nickname` (spec) or `name` (either works; app sends nickname).
    const raw = 'nickname' in body ? body.nickname : body.name
    const out = await mutate(req, p => {
      const mon = ownedMon(p, req.params.id)
      if (!mon) return { status: 404, error: 'Pokémon not found.' }
      const nick = String(raw ?? '').replace(/[\u0000-\u001f|]/g, '').trim().slice(0, 18)
      mon.nickname = nick || null
      return { mon: monView(p, mon) }
    })
    if (out.error) return fail(res, out.status ?? 400, out.error)
  ...
  router.post([`${R}/mons/:id/protect`, `${R}/:id/protect`], requirePlayer, wrap(async (req, res) => {
    const body = req.body ?? {}
    const out = await mutate(req, p => {
      const mon = ownedMon(p, req.params.id)
      if (!mon) return { status: 404, error: 'Pokémon not found.' }
      // Explicit `protected` sets that state; omit it to toggle (matches the
      // chat bot's `.party protect <name>` lock/unlock).
      mon.protected = 'protected' in body ? !!body.protected : !mon.protected
      return { mon: monView(p, mon), protected: !!mon.protected }
    })
    if (out.error) return fail(res, out.status ?? 400, out.error)
    ok(res, out)
  }))

  // POST /mons/:id/give { targetUsername } — gift a Pokémon to another player.
  // Mirrors plugins/pokemon.js handleGive: block self-gift, unregistered target
  // and protected mons. The target is resolved from a NAME server-side; its JID
  // (a phone number) never touches the request or the response.
  router.post([`${R}/mons/:id/give`, `${R}/:id/give`], requirePlayer, wrap(async (req, res) => {
    if (noBattle(req, res)) return
    const typed = String(req.body?.targetUsername ?? req.body?.username ?? '').trim()
    if (!typed) return fail(res, 400, 'targetUsername is required.', { code: 'BAD_REQUEST' })

    const target = findPlayerByName(db, typed)
    if (!target) return fail(res, 404, `No player goes by "${typed}".`, { code: 'TARGET_NOT_FOUND' })
    if (target.id === req.jid) return fail(res, 409, "You can't gift a Pokémon to yourself.", { code: 'SELF_GIFT' })

    // Pre-checks against the live sender, then move the record inside two serial
    // updatePlayer mutators — exactly the give/receive shape handleGive uses.
    const sender = getPlayer(db, req.jid)
    const mon = ownedMon(sender, req.params.id)
    if (!mon) return fail(res, 404, 'Pokémon not found.')
    if (mon.protected) return fail(res, 409, `${mon.nickname ?? mon.name} is protected. Unlock it first.`, { code: 'PROTECTED' })
    if ((sender.pokemon ?? []).length <= 1) return fail(res, 409, "You can't give away your last Pokémon.", { code: 'LAST_POKEMON' })

    let moved = null
    await updatePlayer(db, req.jid, async p => {
      const m = ownedMon(p, req.params.id)
      if (m) {
        moved = m
        p.pokemon = (p.pokemon ?? []).filter(x => x.id !== m.id)
        p.battlePartyIds = partyIds(p).filter(id => id !== m.id)
        if (p.mainPokemonId === m.id) p.mainPokemonId = p.battlePartyIds[0] ?? p.pokemon[0]?.id ?? null
      }
      return p
    })
    if (!moved) return fail(res, 404, 'Pokémon not found.')
    await updatePlayer(db, target.id, async p => {
      if (!Array.isArray(p.pokemon)) p.pokemon = []
      p.pokemon.push(moved)
      return p
    })

    const after = getPlayer(db, req.jid)
    ok(res, {
      gifted: { id: moved.id, name: moved.name, nickname: moved.nickname ?? null, level: moved.level },
      to: targetHandle(target),
      party: partyIds(after).map(id => monView(after, ownedMon(after, id))),
    })
  }))
  ...
  /* ---- party (spec-shaped aliases over battlePartyIds / mainPokemonId) ---- */
  // GET /party — the current battle party (ordered slots) + the main battler.
  // PUT /party (above) replaces the whole list at once; this pair is the
  // per-slot flow the app's party screen drives.
  router.get(`${R}/party`, requirePlayer, wrap(async (req, res) => {
    const p = req.player
    const party = partyIds(p).map(id => ownedMon(p, id)).filter(Boolean)
    ok(res, {
      party: party.map(m => monView(p, m)),
      main: p.mainPokemonId ? (() => { const m = ownedMon(p, p.mainPokemonId); return m ? monView(p, m) : null })() : null,
      partyMax: G.CONST.PARTY_MAX,
      counts: { party: party.length, owned: (p.pokemon ?? []).length },
    })
  }))

  // POST /party { action:'add'|'remove'|'clear'|'main', pokemonId } — mutate one
  // slot. `clear` empties the list (your main still stands in as a 1-mon party
  // via partyIds's fallback, the same invariant the chat bot keeps).
  router.post(`${R}/party`, requirePlayer, wrap(async (req, res) => {
    if (noBattle(req, res)) return
    const action = String(req.body?.action ?? '').toLowerCase()
    const pokemonId = req.body?.pokemonId != null ? String(req.body.pokemonId) : null
    if (!['add', 'remove', 'clear', 'main'].includes(action)) {
      return fail(res, 400, "action must be 'add', 'remove', 'clear' or 'main'.", { code: 'BAD_ACTION' })
    }
    if (action !== 'clear' && !pokemonId) {
      return fail(res, 400, 'pokemonId is required for this action.', { code: 'BAD_REQUEST' })
    }
    const out = await mutate(req, p => {
      const party = partyIds(p)
      if (action === 'clear') {
        p.battlePartyIds = []
        return { party: partyIds(p).map(id => monView(p, ownedMon(p, id))), main: p.mainPokemonId ?? null }
      }
      const mon = ownedMon(p, pokemonId)
      if (!mon) return { status: 404, error: 'Pokémon not found.' }
      if (action === 'add') {
        if (party.includes(mon.id)) return { status: 409, error: 'That Pokémon is already in your party.' }
        if (party.length >= G.CONST.PARTY_MAX) return { status: 409, error: `A party holds at most ${G.CONST.PARTY_MAX} Pokémon.` }
        p.battlePartyIds = [...party, mon.id]
        if (!p.mainPokemonId || !ownedMon(p, p.mainPokemonId)) p.mainPokemonId = mon.id
      } else if (action === 'remove') {
        if (!party.includes(mon.id)) return { status: 409, error: 'That Pokémon is not in your party.' }
        p.battlePartyIds = party.filter(id => id !== mon.id)
        if (p.mainPokemonId === mon.id) p.mainPokemonId = p.battlePartyIds[0] ?? null
      } else if (action === 'main') {
        p.mainPokemonId = mon.id
        if (!party.includes(mon.id)) p.battlePartyIds = [mon.id, ...party].slice(0, G.CONST.PARTY_MAX)
      }
  ...
  /* ---- per-Pokémon care ---- */
  router.post([`${R}/mons/:id/feed`, `${R}/:id/feed`], requirePlayer, wrap(async (req, res) => {
    if (noBattle(req, res)) return
    const leveled = []
    const out = await mutate(req, p => {
      const mon = ownedMon(p, req.params.id)
  ...
  router.post([`${R}/mons/:id/train`, `${R}/:id/train`], requirePlayer, wrap(async (req, res) => {
    if (noBattle(req, res)) return
  ...
  router.post([`${R}/mons/:id/release`, `${R}/:id/release`], requirePlayer, wrap(async (req, res) => {
    if (noBattle(req, res)) return
    const out = await mutate(req, p => {
      const mon = ownedMon(p, req.params.id)
      if (!mon) return { status: 404, error: 'Pokémon not found.' }
