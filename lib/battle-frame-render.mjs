 ...
 */
import { battleOutputTicket, suppressBattleOutput } from './battle-presentation.js'
import { presentResolvedCinematic } from './witch-heroes-cinematic.js'
import { ART } from './witch-heroes.js'
import { rescueChatter } from './guardian-event.js'
...
import { isTitled, ensurePrestige, getTierForXp } from './title-engine.js'
import { titleTextWidth } from './title-glyphs.js'
...
async function tryLoad(file) {
  ...
  if (/^https?:\/\//i.test(mapped)) {
    try {
      const res = await fetch(mapped, { signal: AbortSignal.timeout(8000) })
...
async function resolveBackground(state) {
  // Only approved scene art, never an arbitrary URL from player input.
  if ([ART.endworld1, ART.endworld2].includes(state.cinematicBackground)) {
    const cinematic = await getAsset(state.cinematicBackground)
 ...
 */
/** Trim text with an ellipsis to fit maxW at the ctx's CURRENT font. */
...
function drawNamePlate(ctx, who, x, y, align = 'left') {
  ...
  const metaFont = '10px sans-serif'
  const symbolFont = '10px "Noto Sans Symbols"'
  const metaFill = 'rgba(255,255,255,0.78)'
  const metaAnchorX = align === 'left' ? px + PLATE_W - 7 : px + 7
  const metaAlign = align === 'left' ? 'right' : 'left'
  const nameFont = 'bold 13px sans-serif'

  if (who?.titleGlyph) {
    // Titled player: this 186px plate is already packed tight at the
    // untitled baseline (an 18-char name plus "Lv 200  9999/9999" on one
    // 10px line) — confirmed by rendering it, not assumed: the first pass
    // here truncated the name by a flat character count and still
    // collided with the glyph block for a 16-character name. Budgeting the
    // name's width against the glyph block's REAL measured pixel width
    // (rather than guessing a shorter character cap) is what actually
    // fixes it for every name length, not just the one case that got
    // rendered and checked. The HP bar drawn just below already shows HP
    // visually, so dropping the redundant "9999/9999" numbers from this
    // line when titled also helps reclaim width for the glyph.
    const rest = `Lv ${who.level}`
    ctx.save()
    ctx.font = metaFont
    const restW = ctx.measureText(rest).width
    const glyphW = titleTextWidth(ctx, who.titleGlyph, metaFont)
    ctx.restore()
    const gap = 4
    const metaBlockW = glyphW + gap + restW

    // Available width for the name: plate width minus both 7px paddings
    // minus the meta block minus a small breathing gap between them.
    const nameMaxW = PLATE_W - 14 - metaBlockW - 6
    ctx.save()
    ctx.font = nameFont
    const name = ellipsize(ctx, String(who?.name ?? '???'), Math.max(20, nameMaxW))
    ctx.restore()
    drawLabel(ctx, name, inner, y + 13, { align, font: nameFont })

    if (align === 'left') {
      // Block ends at metaAnchorX: rest (rightmost) then glyph to its left.
      drawLabel(ctx, rest, metaAnchorX, y + 13, { align: 'right', font: metaFont, fill: metaFill })
      drawLabel(ctx, who.titleGlyph, metaAnchorX - restW - gap, y + 13, { align: 'right', font: symbolFont, fill: metaFill })
    } else {
      // Block starts at metaAnchorX: glyph (leftmost) then rest to its right.
      drawLabel(ctx, who.titleGlyph, metaAnchorX, y + 13, { align: 'left', font: symbolFont, fill: metaFill })
      drawLabel(ctx, rest, metaAnchorX + glyphW + gap, y + 13, { align: 'left', font: metaFont, fill: metaFill })
    }
  } else {
    const name = String(who?.name ?? '???').slice(0, 18)
    drawLabel(ctx, name, inner, y + 13, { align, font: nameFont })
    const meta = `${who?.level ? `Lv ${who.level}  ` : ''}${hp}/${maxHp}`
    drawLabel(ctx, meta, metaAnchorX, y + 13, { align: metaAlign, font: metaFont, fill: metaFill })
  }
...
export async function sendBattleTurnReply(ctx, opts) {
  const ticket = battleOutputTicket(ctx, opts)
  if (suppressBattleOutput(ctx, ticket, opts)) return { suppressed: true }
  if (await presentResolvedCinematic(ctx, opts)) return { cinematic: true }

  const { player, e, hpBeforeTurn, eHpBeforeTurn, boss, isPvp, lastAction, isDungeon } = opts
  let { msg } = opts
  // Guardian of the Innocent: the captives are never silent. One line per
  // turn from whoever is still in the pen, slotted in above the HP block.
  if (player?.battleState?.type === 'rescue' && !('lastAction' in opts)) { // intro frames pass lastAction explicitly
    const line = rescueChatter(player.battleState, player, e, { hpBeforeTurn, eHpBeforeTurn })
    if (line) {
      const at = String(msg).lastIndexOf('\n\n👤 *')
      msg = at === -1 ? `${msg}\n\n${line}` : `${msg.slice(0, at)}\n\n${line}${msg.slice(at)}`
    }
  }
  ...
  // Level-200 prestige titles (lib/title-engine.js) on the name plate.
  // `player` (left) is always a real player record. `e` (right) is a real
  // player record too in PvP (plugins/pvp.js passes getPlayer(db, jid), not
  // a stripped copy) but a monster/boss definition otherwise — monsters
  // have no .reborn/.prestige, so only call this when e.isPlayer is set,
  // same guard the rest of this file already uses for "is this a PvP
  // opponent" (see right.isPlayer below).
  const titleGlyphFor = (who) => {
    if (!who) return null
    try {
      const cap = playerLevelCap(who)
      return isTitled(who, cap) ? getTierForXp(ensurePrestige(who).xp).glyph : null
    } catch { return null }
  }

  const state = {
    cinematicBackground: player?.battleState?.cinematicBackground ?? e?.battleState?.cinematicBackground,
    turn,
    isPvp: !!isPvp,
    isBoss,
    isDungeon: !!isDungeon,
    ...
    left: {
      ...
      level: player?.level,
      titleGlyph: titleGlyphFor(player),
    ...
    right: {
      ...
      level: e?.level,
      titleGlyph: e?.isPlayer ? titleGlyphFor(e) : null,
      classId: e?.classId,
      isPlayer: e?.isPlayer,
      tier: e?.tier,
  ...
  // Rendering may have yielded to a cinematic. Never send its stale turn card.
  if (suppressBattleOutput(ctx, ticket, opts)) return { suppressed: true }
