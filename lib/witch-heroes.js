...
export function swordStatus(player) {
  ...
  return Object.entries(SWORD_SKILLS).map(([key, skill]) => {
    const filled = Math.min(charge, skill.charge)
    return `${skill.name}: ${'▰'.repeat(filled)}${'▱'.repeat(skill.charge - filled)} ${filled}/${skill.charge} · ${Math.ceil(Math.max(1, player.maxMp ?? 1) * skill.mp)} MP · .sword ${key}`
  }).join('\n')
}
...
export function useSword(player, target, move) {
  ...
  if (s.charge < skill.charge) return { ok: false, message: `${skill.name} is still charging. Check .ss.` }
  ...
  if (move === 'return') {
    s.returnStroke = true
    return { ok: true, message: 'She turns the blade aside and listens. “Your turn. Make it interesting.”' }
  }
  if (move === 'coordinate') {
  ...
  return { ok: true, damage, message: combo
    ? `“That limp is getting louder.” She reverses the blade through the opening: *${damage}* damage.`
    : move === 'sever' ? `Steel passes through the guard. “Were you hiding behind that?” *${damage}* damage.`
      : `She follows the scrape of a boot and cuts low. “Wrong foot.” *${damage}* damage.` }
}
 ...
 */
export function completeTurn(fighters, turnId) {
  ...
  for (const player of fighters) {
    ...
    if (player.equippedCharacter === 'sword_maiden') s.charge = Math.min(6, s.charge + 1)
