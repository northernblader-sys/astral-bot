 ...
 *
 * Three slips are posted each day, the same three in every hall, seeded like
 * the daily quests: one copper, one iron, one harder. Anyone can read. Taking
 * a slip needs a guild. One active slip. Three turn-ins a day. Pay is Solars
 * and a little fame. A gold slip may add one Gem, and the board never pays a
 * second Gem the same day. Monds are never on a slip. Fame here is a fixed
 * number, not the random kill payout, and it does not enter the treasury.
 *
...
/** The three slips nailed up for this calendar day. Same list for every player. */
export function postedIds(dayStartMs) {
  ...
  return [
    shuffle(of('copper'), day)[0],
    shuffle(of('iron'), day + 17)[0],
    shuffle(hard, day + 91)[0],
  ].filter(Boolean)
}

export function postedSlips(dayStartMs) {
  return postedIds(dayStartMs).map(id => SLIP_BY_ID[id]).filter(Boolean)
}
...
export function rewardTag(slip) {
  const r = slip.reward ?? {}
  const bits = [`☀️ ${r.solars ?? 0}`, `🌟 ${r.fame ?? 0} fame`]
...
export function takeSlip(player, query, now = Date.now()) {
  ...
  const slip = found
  if (!postedIds(state.day).includes(slip.id)) return { ok: false, reason: 'notposted', slip }
...
export function boardView(player, prefix, { motd = '', hall = '' } = {}, now = Date.now()) {
  const state = ensureBoardState(player, now)
  const posted = postedSlips(state.day)
  const lines = [
    `📜 *GUILD BOARD*`,
    `_Sera nailed three slips at dawn. Every hall has the same three. They come down at midnight._`,
  ]
  ...
  if (state.activeId && SLIP_BY_ID[state.activeId]) {
    ...
    lines.push(`_You can read the board. Pulling a slip needs a hall: *${prefix}guild join <name>*._`)
