...
function historyLabel(entry) {
  ...
  switch (entry.event) {
    ...
    case 'rescue':      return `🕊️ Beastkin rescued${n}`
...
/**
 * Award a pre-computed amount of fame (the Guardian of the Innocent rescue
 * uses this: its amount already scales with captives freed and region). Same
 * history/status/tier bookkeeping as awardFame(). Mutates the player.
 */
export function awardFlatFame(player, event, amount, targetName = null) {
  if (!player) return { gained: 0, total: 0, tierChanged: false }
  ...
  if (!Array.isArray(player.fameHistory)) player.fameHistory = []
  player.fameHistory.unshift({ event, value: gained, gained, targetName, at: Date.now() })
  if (player.fameHistory.length > 5) player.fameHistory.length = 5

  if (event === 'rescue') {
    player.fameStatus = `Freed beastkin captives at *${targetName || 'the frontier'}*`
    player.fameLastAction = Date.now()
  }
