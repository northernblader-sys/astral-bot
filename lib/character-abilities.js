...
const KYOKA_MISS_LINES = [
  'The blow lands perfectly. On nothing. The him you struck was never standing there.',
  'Steel passes through a shape that was never him, and finds no one home.',
  'Your eyes watched it connect. Your hands report otherwise.',
  'He is three steps left of the fight your senses are showing you.',
  'The impact feels real right up until it happens.',
  'Whatever you hit, it was the part of the battlefield he was done with.',
]
 ...
 * hypnosis fails, the blow lands whole.
 */
export function applyKyokaSuigetsu(player, rawDamage, bsOverride = null) {
  ...
  if (!complete && Math.random() >= AIZEN_MISDIRECT_CHANCE) {
    return { damage: dmg, message: '', misdirected: false }
  }

  let message = `👁️ ${KYOKA_MISS_LINES[Math.floor(Math.random() * KYOKA_MISS_LINES.length)]}`
  if (st && !complete) {
    st.senses = senses + 1
    const senseName = AIZEN_SENSE_NAMES[senses] ?? 'Sense'
    message += `\n🌀 _Kyōka Suigetsu takes their *${senseName}*._ _(${st.senses}/${AIZEN_MAX_SENSES} senses)_`
