 ...
 */
import { addStatusEffect, hasEffect, getEffectiveStat, resolveVoidRebound, NEGATIVE_EFFECT_TYPES } from './effects.js'
...
import {
  kyokaMisdirectLine, kyokaSenseLine, kyokaCompleteLine, kyokaLandedLine,
  maybeEnemyReaction, hougyokuAwakenLine, hougyokuEnemyReactionLine,
...
export function applyIncomingDamage(player, rawDamage, bsOverride = null) {
  ...
  // Aizen's Kyōka Suigetsu joins the "did the hit happen" tier beside Gogeta's
  // vanish and Circe's guard: a blow his hypnosis misdirects was aimed at a
  // phantom and NEVER ARRIVES, so it must not feed Mei's sustain, spend a
  // Tyla clone-tank, chip the cat pool or burn Yato's tear — same rule those
  // two carry. It is deliberately NOT a reduction like Lovestruck/Infinity:
  // there is no "part of it lands", the enemy swung at an edited battlefield
  // and connected with nothing. Each misdirected blow also costs the enemy one
  // of their five senses (see applyKyokaSuigetsu); at five, the hypnosis is
  // complete and every later blow is misdirected by construction. Only struck
  // damage ever routes here, so poison/burn/bleed/rot still find him — the
  // same honest weakness every perception-based defence in the roster has.
  const kyoka = applyKyokaSuigetsu(player, guard.damage, bsOverride)
  if (kyoka.message) message += (message ? '\n' : '') + kyoka.message
  if (kyoka.misdirected) {
  ...
  const love = resolveLovestruck(player, kyoka.damage, bsOverride)
...
// ── Tella, the Witch of Envy — "Wonders of You" ─────────────────────────────
//
// The first witch, and the most dangerous thing a player can walk a fight next
// to. She adds nothing to her owner's stats and swings no weapon of her own.
// What she does is ENVY, and she does it on a timer, escalating through fixed
// phases the longer her owner is made to trade blows with something that dared
// get close.
//
// THE STATE MACHINE. advanceWondersOfEnvy() is called once per her owner's turn
// in every combat context (PvE attack.js, swarm dungeons, PvP), and walks a
// single latch parked on battleState (bs.envy), exactly like Alexa's
// bs.lovestruck:
//
//   idle      she does not mind. Runs until the exchange has dragged on past
//             ENVY_WARN_AT of her owner's turns.
//   warned    the moment she minds: she HALVES the enemy's current HP and warns
//             them once. The enemy now has ENVY_GRACE_WINDOW moves to stand down.
//   reprieve  granted only if the enemy DEFENDS for ENVY_GRACE_WINDOW moves
//             running (oppDefendedLast, read by the caller). She looks away, and
//             the fight continues for ENVY_REPRIEVE_MOVES more moves.
//   final     her final form. She drains her owner to a single breath and casts
//             a forsaken spell that removes the enemy from the battle outright
//             (a forced loss), and the spell rides out past the bell as a
//             lingering ENVY_CURSE_PCT wound (PvP only; a PvE foe is gone anyway).
//
// WHY A PLAN OBJECT, NOT DIRECT HP WRITES. advanceWondersOfEnvy() never touches
// an enemy record. It returns a PLAN ({ halveFraction, forcedLoss, art, lines })
// and each combat context applies it to whatever shape its enemy takes: a single
// `e` in attack.js, every live monster in a swarm, or the opponent player record
// in a duel. Same separation Alexa uses (resolveLovestruck returns numbers, the
// caller applies them), and the reason she works, and overpowers, in all three
// contexts from one small state machine.
//
// WHO IS IMMUNE. The two world-enders: The End and The Last Prayer. She notes
// once that there is nothing there she envies, and never interferes with that
// fight again. Every other boss, monster and rival's companion is fair game, by
// design: the brief was that she overpowers, bosses included.
export const WITCH_OF_ENVY_CHARACTER_ID = 'witch_of_envy'

// Her final-form art (data/characters.json carries the normal art on .image;
// the ascension splash is sent by the combat contexts when phase hits 'final').
export const WITCH_OF_ENVY_FINAL_FORM_IMAGE = 'https://i.ibb.co/FL0bvjKL/witch-of-envy-final-form.jpg'

// Tuning. All four counts are her owner's-turn counts, not wall-clock. WARN_AT is
// how long she tolerates the fight; GRACE_WINDOW is both the number of defends
// that earn a reprieve AND how long she waits before raging if they are not
// earned; REPRIEVE_MOVES is how long the reprieve lasts before she minds again.
export const ENVY_WARN_AT = 7
export const ENVY_GRACE_WINDOW = 3
export const ENVY_REPRIEVE_MOVES = 4
export const ENVY_HALVE_FRACTION = 0.5
export const ENVY_CURSE_PCT = 0.05

// The two world-event finales, written as bare strings for the same reason
// LOVESTRUCK_IMMUNE_BOSS_ID is (see Alexa's section): this module is imported by
// nearly every combat plugin, and the ids are read defensively across the three
// fields a boss spawn can mark itself with.
const WONDERS_IMMUNE_BOSS_IDS = new Set(['the_end', 'the_last_prayer'])

/** True when `player` has the Witch of Envy equipped. */
export function hasWitchOfEnvy(player) {
  return player?.equippedCharacter === WITCH_OF_ENVY_CHARACTER_ID
}

/** True only for the two world-enders. Nothing else resists her, by design. */
export function bypassesWondersOfEnvy(entity) {
  if (!entity) return false
  return WONDERS_IMMUNE_BOSS_IDS.has(entity.animeBossId)
    || WONDERS_IMMUNE_BOSS_IDS.has(entity.bossId)
    || WONDERS_IMMUNE_BOSS_IDS.has(entity.id)
}

function envyState(bs) {
  return (bs.envy ??= {
    moves: 0, phase: 'idle', warnMove: 0, reprieveStart: 0, defendStreak: 0, immuneNoted: false,
  })
}

// ── Per-context flavor. Three voices: a dungeon monster, a boss that has never
// been envied before, and a rival in a duel. No dashes in any player-facing line.
function envyWarnBlock(ctxKind, foe, owner) {
  if (ctxKind === 'boss') {
    return `🖤 *· WONDERS OF YOU ·*\n` +
      `_${foe} has fought its whole existence to be feared, and never once to be envied. The Witch of Envy fixes on it over ${owner}'s shoulder, and half of the great thing simply withers where it stands._\n` +
      `_"All that power," she says, almost gently, "and none of it was ever yours to keep." She will say it only once. ${foe} would be wise to lower its arms and be still._`
  }
  if (ctxKind === 'pvp') {
    return `🖤 *· WONDERS OF YOU ·*\n` +
      `_Seven times ${foe} has traded blows with ${owner}, and seven times the Witch of Envy let it pass. Her patience has run out._\n` +
      `_She breathes in, and half of ${foe} breathes out and does not come back. "Stop now," she says softly. "Defend yourself, take nothing more, and I may yet look away. Raise a hand against ${owner} again and I promise you I will not."_`
  }
  return `🖤 *· WONDERS OF YOU ·*\n` +
    `_For seven exchanges the Witch of Envy watched ${foe} press in on ${owner}, and did not mind. She minds now._\n` +
    `_Shadow spills off her like slow water. Half of everything ${foe} is stops being, all at once. "You stood too close," she tells it, almost kind. "Step back. I will only ask the once."_`
}

function envyWatchLine(foe) {
  return `🖤 _The Witch of Envy has not blinked. She is still watching ${foe}, waiting to see what it does._`
}

function envyReprieveBlock(foe) {
  return `🖤 *· SHE LOOKS AWAY ·*\n` +
    `_${foe} lowers its guard and covers up, asking for nothing, taking nothing. Slowly the weight of her attention slides off it. "Good," she breathes. The air loosens. The fight goes on, for now._`
}

function envyFinalBlock(ctxKind, foe, owner) {
  const open = `🖤 *· FINAL FORM ·*\n_She warned it. ${foe} did not listen._\n`
  const body = `_The Witch of Envy unfolds out of her own shadow, no longer a woman standing beside ${owner} but a darkness wrapped the whole way around them. She draws ${owner}'s life up into herself until a single thread of it is all that remains, and spends every drop of what she took on one quiet word._\n`
  if (ctxKind === 'boss') {
    return open + body + `_A forsaken spell settles over ${foe} like frost over a grave. Even a thing that had never known fear feels the sentence land. The battle is already decided. Whatever it does now does not count._`
  }
  if (ctxKind === 'pvp') {
    return open + body + `_The forsaken spell finds ${foe} wherever they try to stand. The duel is over. ${owner} has simply been handed the ending._`
  }
  return open + body + `_A forsaken spell settles over ${foe} like frost over a grave. It has already lost. It has only not fallen yet._`
}

function envyImmuneLine(foe) {
  return `🖤 _The Witch of Envy looks at ${foe} for a long moment, and then looks away. There is nothing here she envies. This one, she cannot touch._`
}

/**
 * advanceWondersOfEnvy(bs, opts) -> plan | null
 *
 * Called once per her owner's turn from every combat context. `bs` is the
 * OWNER's battleState (she lives on her owner's latch). opts:
 *   context         'pvp' | 'boss' | 'dungeon' — picks the flavor voice.
 *   foeName         what to call the enemy in the narration.
 *   ownerName       her chosen one's name.
 *   immune          true for The End / The Last Prayer (bypassesWondersOfEnvy).
 *   oppDefendedLast did the enemy DEFEND on their most recent move? Only the PvP
 *                   path can answer true (a monster has no defend action, so a
 *                   PvE foe can never earn the reprieve and marches to the end).
 *
 * Returns null on the many turns where nothing changes. Otherwise a plan:
 *   { halveFraction }              multiply the enemy's current HP by this now.
 *   { forcedLoss, drainOwner,      end the fight against the enemy outright;
 *     cursePct, art }              drain the owner to 1 (PvE), lingering wound.
 *   { lines }                      narration to append, present on every plan.
 */
export function advanceWondersOfEnvy(bs, opts = {}) {
  if (!bs) return null
  const st = envyState(bs)
  if (st.phase === 'done') return null

  const foe = opts.foeName ?? 'the enemy'
  const owner = opts.ownerName ?? 'her chosen one'
  const ctxKind = opts.context === 'pvp' ? 'pvp' : opts.context === 'boss' ? 'boss' : 'dungeon'

  if (opts.immune) {
    if (st.immuneNoted) return null
    st.immuneNoted = true
    st.phase = 'done'
    return { halveFraction: null, forcedLoss: false, lines: envyImmuneLine(foe) }
  }

  st.moves += 1
  if (opts.oppDefendedLast) st.defendStreak += 1
  else st.defendStreak = 0

  // ── idle: she does not mind, until the exchange has dragged on past WARN_AT.
  if (st.phase === 'idle') {
    if (st.moves <= ENVY_WARN_AT) return null
    st.phase = 'warned'
    st.warnMove = st.moves
    st.defendStreak = 0 // the three-move window starts fresh, AFTER the warning
    return { halveFraction: ENVY_HALVE_FRACTION, forcedLoss: false, lines: envyWarnBlock(ctxKind, foe, owner) }
  }

  // ── warned: three moves to stand down (defend), or she takes her final form.
  if (st.phase === 'warned') {
    if (st.defendStreak >= ENVY_GRACE_WINDOW) {
      st.phase = 'reprieve'
      st.reprieveStart = st.moves
      return { halveFraction: null, forcedLoss: false, lines: envyReprieveBlock(foe) }
    }
    if (st.moves - st.warnMove >= ENVY_GRACE_WINDOW) {
      st.phase = 'done'
      return { halveFraction: null, forcedLoss: true, drainOwner: true, cursePct: ENVY_CURSE_PCT, art: WITCH_OF_ENVY_FINAL_FORM_IMAGE, lines: envyFinalBlock(ctxKind, foe, owner) }
    }
    return { halveFraction: null, forcedLoss: false, lines: envyWatchLine(foe) }
  }

  // ── reprieve: she looks away for four moves, then remembers and rages.
  if (st.phase === 'reprieve') {
    if (st.moves - st.reprieveStart >= ENVY_REPRIEVE_MOVES) {
      st.phase = 'done'
      return { halveFraction: null, forcedLoss: true, drainOwner: true, cursePct: ENVY_CURSE_PCT, art: WITCH_OF_ENVY_FINAL_FORM_IMAGE, lines: envyFinalBlock(ctxKind, foe, owner) }
    }
    return null
...
/**
 * armWondersOfEnvy(self, opts) — clear/prime her latch at a fight's opening,
 * beside armLovestruck(). Always clears first so a stale phase from an earlier
 * fight can never ride into a new one on a mid-battle save. No-op for anyone who
 * is not in a fight; primes a fresh idle latch only for her owner. advance*()
 * self-inits too, so arming is belt-and-suspenders, not load-bearing.
 */
export function armWondersOfEnvy(self, opts = {}) {
  const bs = opts.bsOverride ?? self?.battleState
  if (!bs) return null
  bs.envy = null
  if (hasWitchOfEnvy(self)) {
    bs.envy = { moves: 0, phase: 'idle', warnMove: 0, reprieveStart: 0, defendStreak: 0, immuneNoted: false }
  }
  return bs.envy
}

/**
 * applyWondersCurseOnWin(winner, loser) -> line | ''
 *
 * The "even after the battle" half of her forsaken spell, in PvP only. Shaped
 * and called exactly like applyPermanentSeverOnWin(): run inside pvpConclude's
 * loser mutator AFTER the loser is healed to full, so the 5% it removes reads as
 * a wound that visibly did not close. Keyed off loser.wondersCursePending, which
 * the duel's ambient hook stamps at the moment her final form forces the loss,
 * so it only ever fires when she actually forsook them, never on an ordinary KO.
 */
export function applyWondersCurseOnWin(winner, loser) {
  if (!loser) return ''
  const pct = loser.wondersCursePending
  if (!pct || pct <= 0) return ''
  delete loser.wondersCursePending
  const bite = Math.max(1, Math.floor((loser.maxHp ?? 1) * pct))
  loser.hp = Math.max(1, (loser.hp ?? loser.maxHp ?? 1) - bite)
  const pctLabel = Math.round(pct * 100)
  return `🖤 *Wonders of You:* the forsaken spell does not lift with the bell. *${loser.name ?? 'They'}* walk away already down *${bite}* HP _(${pctLabel}% of their total)_, a wound the Witch of Envy left that has not closed.`
}
...
// ── Reverie & Montana ────────────────────────────────────────────────────────
//
// Two sisters, one clock. Reverie STOPS time; Montana is the one thing a stopped
// clock cannot hold.
//
// Reverie (Time Stop) — a once-per-battle active, no mana, .tms
// (plugins/timestop.js). It freezes the target where it stands for
// TIME_STOP_TURNS of its own moves. It computes and applies NOTHING here:
// activateTimeStop() is a pure gate shaped exactly like activatePuppetStrings()
// above (equipped -> in battle -> once-per-battle latch that resets for free
// when battleState clears), and the caller (plugins/timestop.js for PvE, the
// 'timestop' action in plugins/pvp.js for a duel) applies the freeze against the
// writable enemy. The freeze rides the ordinary 'freeze' effect, so the generic
// processStatusTurn() incapacitation branch skips the target's next
// TIME_STOP_TURNS turns for free — no per-plugin enemy-turn hook needed, the
// same discipline Puppet Strings' tangle and Gojo's Unlimited Void both use.
//
// Montana (Faster Than Time) — a PASSIVE, no command, checked live off
// equippedCharacter (so equip/unequip needs no sync flag, unlike Shunya). Two
// halves:
//   1. She tears her own summoner out of any hard control. montanaBreakFree()
//      is called at the top of combat-handlers.js's processStatusTurn() — the
//      one per-turn chokepoint every PvE, boss and duel turn runs through — and
//      strips freeze/stun/sleep off a Montana owner BEFORE the incapacitation
//      check reads them, so she never loses a turn to being held. This is a
//      CLEANSE (the status lands, then breaks the moment her turn comes up), NOT
//      Shunya's prevention (statuses never land at all). That difference is the
//      whole point: the two characters are deliberately distinct.
//   2. When someone stops time, she moves faster than the frozen moment and
//      strikes back for a blow of her own summoner's full strength. That counter
//      is resolved in the 'timestop' branch of plugins/pvp.js (a duel is the
//      only place a time-stop can ever land on a Montana owner — monsters have
//      no equippedCharacter), floored non-lethal like Puppet Strings' redirect
//      so it never one-taps.
export const REVERIE_CHARACTER_ID = 'reverie'
export const MONTANA_CHARACTER_ID = 'montana'

// Turns the target loses to a stopped clock. Three, per the character: "stop
// time for 3 moves."
export const TIME_STOP_TURNS = 3

// Montana breaks her summoner out of EVERY hostile status, not just the holds
// that cost a turn — "any form of stuff like freeze, heat or stuff" per her
// design. She strips the whole NEGATIVE_EFFECT_TYPES set (burn/poison/freeze/
// stun/sleep/blind/weaken/frostlock/tear/shred). This subset is only the
// incapacitating holds, kept so the reveal line can tell "wrenched out of a
// freeze" apart from "shrugged off a burn" — and kept in lockstep with the
// incapacitation check in combat-handlers.js's processStatusTurn().
const MONTANA_HARD_CC = ['freeze', 'stun', 'stun_chance', 'sleep']

// Montana's duel counter never finishes the caster — it stops at this share of
// their max HP, the same non-lethal discipline Puppet Strings' redirect uses
// (PUPPET_FLOOR_PCT). She hits back hard, she does not kill through a .tms.
const MONTANA_COUNTER_FLOOR_PCT = 0.20

/** True when `player` has Reverie (Time Stop) equipped. */
export function hasReverie(player) {
  return player?.equippedCharacter === REVERIE_CHARACTER_ID
}

/** True when `player` has Montana (Faster Than Time) equipped. */
export function hasMontana(player) {
  return player?.equippedCharacter === MONTANA_CHARACTER_ID
}

/**
 * activateTimeStop(player, bsOverride) -> { ok, message }
 *
 * Pure gate, identical in shape to activatePuppetStrings(): validates the
 * character, the battle, and the once-per-battle latch, and burns the charge on
 * success. It computes and applies NOTHING — the caller freezes the writable
 * enemy for TIME_STOP_TURNS, so PvE and PvP each own their own write and defeat
 * check without this function needing a writable enemy. A failed gate never
 * spends the charge.
 */
export function activateTimeStop(player, bsOverride = null) {
  const bs = bsOverride ?? player.battleState
  if (!hasReverie(player)) {
    return { ok: false, message: `❌ You need *Reverie* equipped to use *.tms*.` }
  }
  if (!player.inBattle || !bs) {
    return { ok: false, message: `❌ Not in battle.` }
  }
  const gate = ultimateGate(player, bs, 'timeStop', 'Time Stop',
    `⏱️ *Time has already stopped once this battle.*\n` +
    `_The clock only holds its breath the one time. It is running again now._`)
  if (gate.blocked) return { ok: false, message: gate.message }
  burnUltimate(player, bs, 'timeStop')
  return { ok: true, message: null }
}

/**
 * montanaBreakFree(entity) -> { broke: true, line } | null
 *
 * Montana's passive cleanse. If `entity` has Montana equipped and is carrying
 * ANY hostile status (the whole NEGATIVE_EFFECT_TYPES set — a Reverie time-stop
 * rides 'freeze', a heatwave rides 'burn', and so on), every one of them is
 * stripped off her summoner and a reveal line is returned; otherwise null.
 * Called from processStatusTurn() BEFORE the incapacitation check, so the strip
 * decides that turn's outcome — a held summoner acts anyway. The reveal reads
 * differently for a turn-costing hold (freeze/stun/sleep) than for a plain
 * debuff or DoT. Mutates entity.activeEffects in place; a no-op for monsters and
 * for anyone without her equipped, exactly like the fusion clock beside it.
 */
export function montanaBreakFree(entity) {
  if (!hasMontana(entity)) return null
  const fx = entity?.activeEffects
  if (!Array.isArray(fx) || !fx.length) return null
  const held = fx.filter(e => NEGATIVE_EFFECT_TYPES.has(e.type) && (e.remaining ?? 0) > 0)
  if (!held.length) return null
  entity.activeEffects = fx.filter(e => !(NEGATIVE_EFFECT_TYPES.has(e.type) && (e.remaining ?? 0) > 0))
  const name = entity.name ?? 'her summoner'
  const line = held.some(e => MONTANA_HARD_CC.includes(e.type))
    ? `🕰️ *Montana* wrenches *${name}* out of the frozen moment before it can close. The hold shatters and she acts anyway.`
    : `🕰️ *Montana* moves before the affliction can settle on *${name}*, and it sloughs off unspent.`
  return { broke: true, line }
}
...
/**
 * resolveMontanaCounter(attacker, target) -> { raw, dealt, newHp, floored, floorHp }
 *
 * Montana's duel counter. When someone stops time on her summoner, she moves
 * faster than the frozen moment and strikes the caster back for a swing built
 * from her summoner's OWN offence ("equal strength and stats to her own user, so
 * she deals equal damage") run through the caster's defense. Same non-writing,
 * never-lethal shape as resolvePuppetSelfHit: the caller sets target.hp =
 * res.newHp, and MONTANA_COUNTER_FLOOR_PCT of the caster's max HP is the hard
 * floor, so a .tms into a Montana owner can never itself finish the caster. No
 * PUPPET_SELF_MULT here — a flat 1.0 swing, because "equal damage" means her
 * summoner's blow, not a boosted one.
 */
export function resolveMontanaCounter(attacker, target) {
  const atk     = puppetSelfNumbers(attacker).atk
  const def     = puppetSelfNumbers(target).def
  const maxHp   = Math.max(1, Math.floor(Number(target?.maxHp ?? target?.hp ?? 1)))
  const hp      = Math.max(0, Math.floor(Number(target?.hp ?? 0)))
  const raw     = Math.max(1, Math.floor(applyDefense(atk, def)))
  const floorHp = Math.floor(maxHp * MONTANA_COUNTER_FLOOR_PCT)
  const newHp   = Math.min(hp, Math.max(floorHp, hp - raw))
  const dealt   = Math.max(0, hp - newHp)
  return { raw, dealt, newHp, floored: dealt < raw, floorHp }
}
...
// ── Aizen Sosuke — Kyōka Suigetsu ──────────────────────────────────────────
//
// "Complete Hypnosis" — the most layered kit in the roster after Gojo's, and
// deliberately NOT another dodge/illusion. Three interlocking pieces:
//
//   1. PASSIVE — the misdirect. Every struck hit against his owner is aimed at
//      a phantom (flat AIZEN_MISDIRECT_CHANCE, ANY size — this is the point of
//      difference: Gojo's Infinity gates on hit SIZE, Alexa's Lovestruck on a
//      power tier computed at the bell, Gogeta's Transmission on a vanish roll
//      at the very front. Aizen's is about PERCEPTION: a coin-flip on every
//      single blow, small or huge, and what fails to land teaches him the
//      enemy). A misdirected blow costs the enemy one of their five senses
//      (bs.aizen.senses, capped at 5).
//
//   2. THE SENSE ARC — the complexity. Senses are not just a counter: they are
//      the fuel for Kurohitsugi and the runway to the endgame state. At 5/5
//      the hypnosis is COMPLETE — every struck blow is misdirected from then
//      on (chance becomes 1), and it stays that way until the fight ends. A
//      long fight against Aizen is a fight the enemy slowly loses the ability
//      to perceive at all. The state lives on battleState (bs.aizen), so it
//      resets with the fight for free, exactly like every other per-battle
//      latch — and it is per-DEFENDER, so in a duel each Aizen (there can be
//      only one, he is one-of-one) tracks his own arc on his own battleState.
//
//   3. THE TWO ACTIVES — plugins/kurohitsugi.js and plugins/hogyoku.js, with
//      pvp handoffs in plugins/pvp.js (runPvpTurn actions 'kurohitsugi' /
//      'hougyoku'). Wager duels never see any of it: resolveWagerAction()
//      deliberately excludes applyIncomingDamage()'s character-passive layer
//      and runPvpTurn refuses character actions outright in a wager state —
//      so this kit is live in dungeon runs, boss fights and ordinary duels
//      ONLY, which is the spec.
//
// Senses are stored as a plain count plus the completion flip; the sense NAMES
// are presentation, drawn in order (Sight, Hearing, Touch, Smell, Taste) from
// AIZEN_SENSE_NAMES, matching the five senses Kyōka Suigetsu canonically owns.
export const AIZEN_CHARACTER_ID = 'aizen'
export const AIZEN_MAX_SENSES = 5
export const AIZEN_MISDIRECT_CHANCE = 0.30
export const AIZEN_SENSE_NAMES = ['Sight', 'Hearing', 'Touch', 'Smell', 'Taste']

// Kurohitsugi (Hadō #90) — the closer. Base multiplier sits under Hollow
// Purple's flat 18 (that one erases whatever it points at, once per battle,
// and is the game's hardest guaranteed hit) and above Cinder Verdict's flat 8.
// The complexity is the two scales it rides: +KUROHITSUGI_PER_SENSE per stolen
// sense (the more of their world he already owns, the tighter the coffin) and
// +KUROHITSUGI_EXECUTE_MULT at enemy death's door (it crushes what is already
// breaking). 0 senses + full-HP enemy is the floor; 5 senses + a dying enemy
// is the hardest single hit in the game (≈24x), which is the payoff for
// letting the fight run long enough to complete the hypnosis. Applied RAW —
// no DEF, never misses — same contract as Hollow Purple.
export const KUROHITSUGI_BASE_MULT = 6
export const KUROHITSUGI_PER_SENSE = 2
export const KUROHITSUGI_EXECUTE_MULT = 8

// Hōgyoku (The One Above All) — the comeback. Once per battle: his body
// reforges (HOUGYOKU_HEAL_PCT of maxHp), ALL five senses fall at once
// (complete hypnosis, immediately — the arc's fast-forward), and every stat
// surges HOUGYOKU_STAT_MULT for the rest of the battle (a permanent-for-the-
// battle strengthen on str/agi/int/def, the exact Final Form pattern: duration
// 999 and discarded with battleState when the fight ends). It deals no damage;
// the turn is spent on evolving past the ceiling.
export const HOUGYOKU_HEAL_PCT = 0.35
export const HOUGYOKU_STAT_MULT = 0.35

// The narration pools themselves live in lib/aizen-flavor.js (misdirects,
// sense steals, the completion moment, the enemy's reaction, and the lines
// for the blows that DO find him). This file only decides WHEN a moment
// happens; what it sounds like is the flavor module's job.

/** True when `player` has Aizen (Kyōka Suigetsu) equipped. */
export function hasAizen(player) {
  return player?.equippedCharacter === AIZEN_CHARACTER_ID
}

/** Lazily create the per-battle Aizen state on `bs`; null when there is no bs. */
function aizenState(bs) {
  if (!bs || typeof bs !== 'object') return null
  if (!bs.aizen || typeof bs.aizen !== 'object') bs.aizen = { senses: 0 }
  if (typeof bs.aizen.senses !== 'number') bs.aizen.senses = 0
  return bs.aizen
}

/** Senses stolen so far (0-5). Read-only. */
export function aizenSenses(player, bsOverride = null) {
  const bs = bsOverride ?? player?.battleState
  return bs?.aizen?.senses ?? 0
}

/** True once all five senses are his: every blow misdirected from here. */
export function isCompleteHypnosis(player, bsOverride = null) {
  return aizenSenses(player, bsOverride) >= AIZEN_MAX_SENSES
}

/**
 * applyKyokaSuigetsu(player, rawDamage, bsOverride) -> { damage, message, misdirected }
 *
 * The passive half, called from applyIncomingDamage() on the defender at the
 * "did the hit happen" tier (after Gogeta's vanish and Circe's guard). Returns
 * the raw damage untouched for anyone who is not Aizen. For Aizen: the blow is
 * rolled against the hypnosis: a misdirect returns damage 0 (the hit never
 * arrived, so the caller skips the rest of the pipeline exactly like a vanish)
 * and steals one more sense while senses < 5; at 5 the roll is skipped and
 * every blow misdirects. A hit that ISN'T misdirected passes through at full
 * size: he has no reduction layer at all, which is his honest trade: when the
 * hypnosis fails, the blow lands whole. Either way the message now carries
 * flavor from lib/aizen-flavor.js: the misdirect and the sense steal when the
 * hypnosis holds, a reaction beat when a blow pierces it, so every struck hit
 * against him reads like the fight it is in all four battle forms.
 */
export function applyKyokaSuigetsu(player, rawDamage, bsOverride = null) {
  if (!hasAizen(player)) return { damage: rawDamage, message: '', misdirected: false }
  const dmg = Math.max(0, Math.round(rawDamage))
  if (dmg <= 0) return { damage: rawDamage, message: '', misdirected: false }

  const bs = bsOverride ?? player.battleState
  const st = aizenState(bs)
  const senses = st?.senses ?? 0
  const complete = senses >= AIZEN_MAX_SENSES
  if (!complete && Math.random() >= AIZEN_MISDIRECT_CHANCE) {
    // The hypnosis slipped: the blow lands whole, and BOTH sides react. The
    // enemy pierced it this once, which is exactly the rare event worth
    // narrating (see kyokaLandedLine in lib/aizen-flavor.js).
    return { damage: dmg, message: kyokaLandedLine() + maybeEnemyReaction(), misdirected: false }
  }

  let message = kyokaMisdirectLine()
  if (st && !complete) {
    st.senses = senses + 1
    const senseName = AIZEN_SENSE_NAMES[senses] ?? 'Sense'
    message += `\n${kyokaSenseLine(senseName, st.senses, AIZEN_MAX_SENSES)}`
    if (st.senses >= AIZEN_MAX_SENSES) {
      message += `\n${kyokaCompleteLine()}`
    }
  }
  message += maybeEnemyReaction()
  return { damage: 0, message, misdirected: true }
}

/**
 * .kurohitsugi gate — validates + burns the once-per-battle charge (bs.kurohitsugiUsed),
 * mirroring activateHollowPurple(). Carries no multiplier: the number depends
 * on the enemy's live state, so callers ask kurohitsugiMultiplier() at hit time.
 */
export function activateKurohitsugi(player, bsOverride = null) {
  const bs = bsOverride ?? player.battleState
  if (!hasAizen(player)) {
    return { ok: false, message: `❌ You need *Aizen* equipped to use *.kurohitsugi*.` }
  }
  if (!player.inBattle || !bs) {
    return { ok: false, message: `❌ Not in battle.` }
  }
  const gate = ultimateGate(player, bs, 'kurohitsugi', 'Kurohitsugi')
  if (gate.blocked) return { ok: false, message: gate.message }
  burnUltimate(player, bs, 'kurohitsugi')
  return { ok: true, message: null }
}

/**
 * kurohitsugiMultiplier(player, enemy, bsOverride) -> number
 *
 * The coffin's two scales, folded into one multiplier the shared hit math can
 * carry: base, +per stolen sense, +execute share scaled by the enemy's missing
 * HP (0 at full HP, the whole KUROHITSUGI_EXECUTE_MULT at death's door). The
 * caller applies it RAW — no applyDefense(), no accuracy roll — same contract
 * as Hollow Purple.
 */
export function kurohitsugiMultiplier(player, enemy, bsOverride = null) {
  const senses = aizenSenses(player, bsOverride)
  const maxHp = enemy?.maxHp ?? 0
  const missing = maxHp > 0
    ? Math.max(0, Math.min(1, 1 - (enemy.hp ?? maxHp) / maxHp))
    : 0
  return KUROHITSUGI_BASE_MULT
    + KUROHITSUGI_PER_SENSE * Math.min(senses, AIZEN_MAX_SENSES)
    + KUROHITSUGI_EXECUTE_MULT * missing
}

/**
 * .hougyoku gate + resolution — validates and burns the once-per-battle charge
 * (bs.hougyokuUsed), then evolves him on the spot: heal, five senses, and the
 * rest-of-battle stat surge (the Final Form strengthen pattern: duration 999,
 * discarded with battleState). Self-contained on `player`, so it resolves fine
 * inside the actor's own updatePlayer() mutator in both PvE and PvP. No damage.
 */
export function activateHogyoku(player, bsOverride = null) {
  const bs = bsOverride ?? player.battleState
  if (!hasAizen(player)) {
    return { ok: false, message: `❌ You need *Aizen* equipped to use *.hougyoku*.` }
  }
  if (!player.inBattle || !bs) {
    return { ok: false, message: `❌ Not in battle.` }
  }
  const gate = ultimateGate(player, bs, 'hougyoku', 'Hōgyoku')
  if (gate.blocked) return { ok: false, message: gate.message }
  burnUltimate(player, bs, 'hougyoku')

  const healAmount = Math.floor((player.maxHp ?? 0) * HOUGYOKU_HEAL_PCT)
  const before = player.hp
  player.hp = Math.min(player.maxHp, (player.hp ?? 0) + healAmount)
  const healed = player.hp - before

  const st = aizenState(bs)
  const sensesBefore = st?.senses ?? 0
  if (st) st.senses = AIZEN_MAX_SENSES

  // Rest-of-battle surge, one strengthen per stat he actually carries — same
  // shape (and same duration-999-with-no-teardown rule) as activateFinalForm.
  const boosted = []
  for (const stat of ['str', 'agi', 'int', 'def']) {
    const delta = Math.round((player.stats?.[stat] ?? 0) * HOUGYOKU_STAT_MULT)
    if (delta > 0) {
      addStatusEffect(player, { type: 'strengthen', stat, value: delta, duration: 999, sourceId: 'hougyoku' })
      boosted.push(stat)
    }
  }

  const senseLine = sensesBefore >= AIZEN_MAX_SENSES
    ? `🌀 The hypnosis was already complete: the Hōgyoku only agrees.`
    : `🌀 The five senses fall at once: *COMPLETE HYPNOSIS*, here and now.\n_What remains of the fight happens entirely inside a world he authored._`
  ...
  return {
    ...
    message:
      ...
      `─────────────\n` +
      `${hougyokuAwakenLine()}\n\n` +
      `❤️ Reforged *${healed}* HP! (${player.hp}/${player.maxHp})\n` +
      `_${senseLine}_\n` +
      (boosted.length ? `⚔️ Strength, speed, mind and guard surge *+${Math.round(HOUGYOKU_STAT_MULT * 100)}%* for the rest of the battle.\n` : '') +
      `${hougyokuEnemyReactionLine()}\n` +
      `_Whatever he is now, it is out of the enemy's reach entirely._`,
  }
}
...
// Her full kit lives in lib/echidna.js (standalone module, same split
// precedent as yoriichi.js / megumi.js / gogeta.js); this block re-publishes
// those exports so combat files importing everything from here keep working,
// and owns the ONE piece that must live here: the battle gate, which needs
// ultimateGate()/burnUltimate() - private to this file.
//
// Gospel of Greed (.greed): once per battle, no MP. Her MOOD decides the
// size of the theft (amused 45% / capricious 35% / displeased 20%): half the
// enemy's money when she is amused or capricious (plus a gem chance), a
// quarter when displeased. The enemy also loses its next turn, distracted
// patting its empty pockets (a plain stun, same shape as Puppet Strings'
// tangle). In PvP the money comes straight off the opponent's wallet and the
// gems are stolen from the opponent themselves - see plugins/greed.js (PvE)
// and pvp.js's 'greedtithe' branches. Her holder's granted child steals
// alongside her via childBattleBonus() (lib/echidna-child.js).
export {
  ECHIDNA_CHARACTER_ID,
  GREED_TANGLE_TURNS,
  GREED_TITHE_LATCH,
  ECHIDNA_MOODS,
  hasEchidna,
...
import {
  hasEchidna as _echidnaEquipped,
  rollEchidnaMood as _rollEchidnaMood,
  GREED_TITHE_LATCH as _GREED_TITHE_LATCH,
} from './echidna.js'

/**
 * activateGreedTithe(player, bsOverride?) -> { ok, message, mood? }
 *
 * Pure gate, identical in shape to activateTimeStop(): validates the
 * character, the battle, and the once-per-battle latch, burns the charge on
 * success and rolls her mood for the caller to run through the tithe math.
 * Computes and writes NOTHING else - PvE (plugins/greed.js) and PvP
 * (plugins/pvp.js) each own their own wallet writes and the distraction
 * stun. A failed gate never spends the charge.
 */
export function activateGreedTithe(player, bsOverride = null) {
  const bs = bsOverride ?? player.battleState
  if (!_echidnaEquipped(player)) {
    return { ok: false, message: `❌ You need *Echidna* equipped to use *.greed*.` }
  }
  if (!player.inBattle || !bs) {
    return { ok: false, message: `❌ Not in battle.` }
  }
  const gate = ultimateGate(player, bs, 'greedTithe', 'Gospel of Greed',
    `🍵 *The Gospel has already been opened once this battle.*\n` +
    `_Her mood was spent on the first theft. It does not open twice._`)
  if (gate.blocked) return { ok: false, message: gate.message }
  burnUltimate(player, bs, 'greedTithe')
  return { ok: true, mood: _rollEchidnaMood(), message: null }
}
