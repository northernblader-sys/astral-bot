...
export function calcPlayerDamage(
  ...
  const rawDmg = Math.floor(
    base * mult * critMult * damageMultiplier * rebornWeaponMultiplier(player) * warcryMult,
  )
...
export function applyLevelUps(player, levelsData, classes, races, getTotalStats) {
  const msgs = []
  // Per-player ceiling: 100 normally, 150 once the player has been reborn
  // (see plugins/reborn.js and lib/reborn-engine.js). data/levels.json's
  // xpTable runs all the way to 150, so nothing else here needs to change.
