...
// ── Per-location difficulty overrides ──────────────────────────────────────
// A location's entry into REGULAR_MONSTER_BALANCE. The keys ARE the global
// balance constants, so an override reads as "same dial, different number" and
// cannot grow a parallel maths path; anything not listed keeps the global
// value, which is why every other dungeon in the roster is bit-for-bit
// untouched by this table. Three keys are location-only and consumed at their
// own call sites: `defFloorMultiplier` (raises raw DEF, which the global
// `maxMitigation` alone could not, since that is only a ceiling),
// `eliteChance` (pickMonsterForFloor) and `playerScale` (scaleEnemyToPlayer).
//
// caged_dimension: GODLY (2026-09-22). The dimension is described in
// data/locations.json as "sealed away because nothing else could contain them"
// and pays 10x XP / 8x Solars for it, but the numbers said otherwise: measured
// with scripts/check-monster-power.mjs it took a floor-level player 3 hits to
// die and 6 to kill a monster, i.e. the same shape as eternal_dungeon two
// floors down, because the global curve is *capped* at clearTurnsDeep 6 and
// survivalTurnsDeep 3.5 by design. Eight monsters' authored stats could not
// fix that; the dials that could are these. What they mean in play: a monster
// lands enough to gut a geared level-200 hunter in two exchanges, it costs
// something like two dozen of the player's own attacks to put down, it wears
// armour that actually cuts (DEF was pinned to the roster's own 30 to 150
// before this, which applyDefense turns into a rounding error at level 200),
// and nearly half of them are elites. It also stops shrinking for the players
// it used to shrug off: `playerScale` raises the exponents in
// scaleEnemyToPlayer so a whale who outgrew the curve gets a proportionally
// bigger enemy instead of the global squishy 0.85/0.4/0.2 profile.
//
// Pinned by test/caged-dimension.test.mjs, which asserts BOTH halves: that the
// dimension is brutal, and that the four swarm dungeons plus entry tower still
// balance off the untouched globals. Do not "fix" the wall by editing
// REGULAR_MONSTER_BALANCE itself.
export const LOCATION_DIFFICULTY = {
  caged_dimension: {
    // Monster attacks the reference player can absorb. Global: 9 falling to 3.5.
    survivalTurnsShallow: 2.4,
    survivalTurnsDeep:    1.7,
    // Player attacks one monster costs. Global: 4 rising to 6, and 6 is the
    // ceiling this zone exists to break.
    clearTurnsShallow: 19,
    clearTurnsDeep:    23,
    // A hitter in the pool is allowed to hit even harder relative to a wall,
    // so "Void Serpent" and "Shatter Golem" are different fights.
    archetypeSpread: 0.35,
    // Armour: the ceiling on mitigation, and the multiplier on the roster's
    // own raw DEF (80 x 6 = 480, which is ~49% of the player's damage gone).
    maxMitigation: 0.72,
    defFloorMultiplier: 6,
    // Elites are 15% everywhere else; here the ceiling of the dimension IS
    // Ruin Shade, so it should be showing up. Its multipliers deliberately
    // stay at the global 1.6 HP / 1.15 ATK rather than being raised too: those
    // multiply a clear target that is already ~4x the roster's, and compounding
    // them would push an elite past fifty of the player's own attacks, which is
    // a broken fight rather than a hard one. Frequency, not a second multiplier.
    eliteChance: 0.45,
    // Over-levelled players: hp/atk/def exponents used instead of the
    // MONSTER_SCALE_1V1 / MONSTER_SCALE profiles when the enemy's power ratio
    // to the floor exceeds 1.
    playerScale: { hpExp: 1.0, atkExp: 0.7, defExp: 0.35 },
  },
}
...
function balanceRegularMonsterStats(def, floor, rawHp, rawAtk, rawDef) {
  // Per-location overrides win over the globals, and everything else in the
  // roster reads the globals untouched. See LOCATION_DIFFICULTY above.
  const locOverride = LOCATION_DIFFICULTY[def.locationId]
  const B = locOverride
    ? { ...REGULAR_MONSTER_BALANCE, ...locOverride }
    : REGULAR_MONSTER_BALANCE
  ...
  // `defFloorMultiplier` is the one lever a location has to make its monsters
  // genuinely armoured rather than merely capped: the caged_dimension roster
  // authors DEF between 30 and 150 with zero per-floor scaling, so without
  // raising it the cap above is never approached and the zone reads unarmoured
  // no matter how high maxMitigation goes.
  const defCap = Math.round(500 * B.maxMitigation / (1 - B.maxMitigation))
  const rawDefFloor = B.defFloorMultiplier && B.defFloorMultiplier > 1
    ? Math.round(rawDef * B.defFloorMultiplier)
    : rawDef
  const monsterDef = Math.max(0, Math.min(rawDefFloor, defCap))
...
export function pickMonsterForFloor(locationId, floor, regularByLoc) {
  ...
  let chosen
  let isElite = false
  // 15% everywhere, 45% in the Caged Dimension (LOCATION_DIFFICULTY.eliteChance).
  const eliteChance = LOCATION_DIFFICULTY[locationId]?.eliteChance ?? 0.15
  if (elites.length && Math.random() < eliteChance) {
...
export function scaleEnemyToPlayer(enemy, ratio, opts = {}) {
  if (!enemy || !(ratio > 1)) return enemy
  // A location can demand a harder scaling profile than the caller's default:
  // caged_dimension uses steeper exponents so the enemy an over-levelled whale
  // meets keeps pace with them (see LOCATION_DIFFICULTY.playerScale). Applied
  // here rather than at each call site because every path (plugins/dungeon.js's
  // 1v1, lib/swarm-combat.js's packs) funnels through this one function, and
  // the enemy already carries the locationId the table is keyed by.
  const locScale = LOCATION_DIFFICULTY[enemy.locationId]?.playerScale
  const {
    hpExp = 0.85, atkExp = 0.5, defExp = 0.3,
    atkCap = Infinity, defCap = Infinity,
  } = locScale ? { ...opts, ...locScale } : opts
...
// ─────────────────────────────────────────────────────────────────────────────
// Veteran difficulty: the monsters a LEVEL 100+ player meets
// ─────────────────────────────────────────────────────────────────────────────
// The floor curve and the power-ratio scaling above both work off how strong a
// player is *relative to the floor they are standing on*. That leaves one band
// untouched: a player at the level cap who has NOT outgrown the content (a
// fresh level-100 in Eternal Dungeon gear, a reborn character re-walking a
// tower) is exactly "on curve" — so nothing scales, and the report from that
// band was that the game gets easier the further you get, not harder. Reborn
// levels go to 200 while every dungeon's expectations stop at 100, so the whole
// second half of the level track was fighting foes tuned for level 100.
//
// This is a deliberate, announced difficulty tier rather than a hidden nerf:
// from `fromLevel` up, the monsters a player meets carry more HP, a capped
// attack bump and a small armor bump, growing with every level past 100 and
// capped so the last floors never become a war of attrition. It applies AFTER
// the ratio scaling (they solve different problems: ratio = "this player is
// stronger than this floor expects"; veteran = "this player is past the level
// the whole game was tuned for"), and it is skipped entirely below the
// threshold, so nothing about the 1-99 climb changes.
//
// All three curves are env-tunable (VETERAN_MONSTER_* below) precisely because
// this is the one knob a live game needs to feel out.
const envNumber = (name, fallback) => {
  const raw = Number(process.env[name])
  return Number.isFinite(raw) && raw > 0 ? raw : fallback
}

export const VETERAN_SCALE = {
  // Level at which the tier starts. The vanilla cap is 100 (data/levels.json),
  // reborn cap 200 — so this covers the whole band the floor curve ignored.
  fromLevel:     envNumber('VETERAN_MONSTER_LEVEL', 100),
  // HP is the fight-length lever (matches scaleEnemyToPlayer's reasoning): the
  // bump at exactly fromLevel, then per level past it, hard-capped.
  hpBase:        envNumber('VETERAN_MONSTER_HP_BASE', 1.25),
  hpPerLevel:    envNumber('VETERAN_MONSTER_HP_PER_LEVEL', 0.02),
  hpMax:         envNumber('VETERAN_MONSTER_HP_MAX', 2.6),
  // ATK climbs gently: a veteran should take real damage, never a one-shot
  // through a full tank build. The slope is chosen so the reborn cap (level
  // 200) lands exactly on all three ceilings rather than just short of them.
  atkBase:       envNumber('VETERAN_MONSTER_ATK_BASE', 1.15),
  atkPerLevel:   envNumber('VETERAN_MONSTER_ATK_PER_LEVEL', 0.0065),
  atkMax:        envNumber('VETERAN_MONSTER_ATK_MAX', 1.8),
  // DEF the least and lowest: applyDefense's curve means armor eats damage
  // fast, and the goal is a longer, meaner fight — not a slog against a wall.
  defBase:       envNumber('VETERAN_MONSTER_DEF_BASE', 1.10),
  defPerLevel:   envNumber('VETERAN_MONSTER_DEF_PER_LEVEL', 0.004),
  defMax:        envNumber('VETERAN_MONSTER_DEF_MAX', 1.5),
}

/**
 * How hard the veteran tier bites for `player`, or null when it doesn't apply.
 * Returns the multipliers rather than applying them, so callers that already
 * scale enemies (the swarm's per-pack profile) can spend them differently.
 */
export function veteranTier(player) {
  const level = Math.max(0, Math.floor(Number(player?.level) || 0))
  if (level < VETERAN_SCALE.fromLevel) return null
  const steps = level - VETERAN_SCALE.fromLevel
  const V = VETERAN_SCALE
  // Rounded to 4 decimals: the ramps are floats, and 1.15 + 0.0065 * 100 lands
  // on 1.7999999999999998 — which reads wrong in the encounter banner and never
  // quite equals the cap it is supposed to reach.
  const ramp = (base, per, max) => Math.round(Math.min(max, base + per * steps) * 1e4) / 1e4
  return {
    level,
    steps,
    hpMult:  ramp(V.hpBase,  V.hpPerLevel,  V.hpMax),
    atkMult: ramp(V.atkBase, V.atkPerLevel, V.atkMax),
    defMult: ramp(V.defBase, V.defPerLevel, V.defMax),
  }
}

/** One-line banner for encounter text, or '' when the tier isn't in play. */
export function veteranBanner(player) {
  const tier = veteranTier(player)
  if (!tier) return ''
  return (
    `🔥 *Veteran pace* — past Level ${VETERAN_SCALE.fromLevel}, monsters hold ` +
    `*${tier.hpMult.toFixed(2)}x* HP and hit for *${tier.atkMult.toFixed(2)}x* damage.`
  )
}

/**
 * Apply the veteran tier to one enemy, in place.
 *
 * `weights` scales how much of each bump this enemy spends, as a fraction of
 * the EXCESS over 1 (weight 0.5 = half the bump, 0 = untouched). The swarm path
 * uses partial weights because its monsters already arrive in packs of two to
 * four — a full HP bump on each of four monsters is four times the wall, not
 * one. Bosses and 1v1 encounters take it in full.
 */
export function applyVeteranScaling(enemy, player, weights = {}) {
  const tier = veteranTier(player)
  if (!enemy || !tier) return enemy
  const { hp = 1, atk = 1, def = 1 } = weights
  const spend = (mult, weight) => 1 + (mult - 1) * Math.max(0, weight)
  const hpMul  = spend(tier.hpMult,  hp)
  const atkMul = spend(tier.atkMult, atk)
  const defMul = spend(tier.defMult, def)
  ...
  if (atkMul !== 1) {
    if (enemy.atk     != null) enemy.atk     = Math.max(1, Math.round(enemy.atk     * atkMul))
    if (enemy.baseAtk != null) enemy.baseAtk = Math.max(1, Math.round(enemy.baseAtk * atkMul))
  }
  if (defMul !== 1) {
    if (enemy.def     != null) enemy.def     = Math.max(0, Math.round(enemy.def     * defMul))
    if (enemy.baseDef != null) enemy.baseDef = Math.max(0, Math.round(enemy.baseDef * defMul))
  }
  // Stamped so every downstream reader (swarm frames, reward text, .health) can
  // tell a veteran-scaled enemy apart from an authored one without recomputing.
  enemy.veteran = { level: tier.level, hpMult: hpMul, atkMult: atkMul, defMult: defMul }
  return enemy
}
