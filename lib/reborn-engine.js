 ...
 * 100 to 200, every stat gains +100, max HP gains +300, and they choose one
...
/** The player's personal level ceiling: 100 normally, 200 once reborn. */
export function playerLevelCap(player) {
  const base = levelsData.levelCap ?? 100
  if (!isReborn(player)) return base
  return Math.max(base, levelsData.rebornLevelCap ?? 200)
}
...
export function playerMaxStatPoints(player) {
  ...
  if (!isReborn(player)) return base
  return Math.max(base, levelsData.rebornMaxStatPoints ?? 3000)
}
