...
export function completeChapter(player, volume, volumeId, chapter, rewards) {
  ...
  if (gemsAward > 0) {
    // A partial save (no wallet block) used to throw right here, in the middle of
    // the updatePlayer() mutator, which killed the whole chapter completion and
    // left the player mid-chapter with nothing posted to the group. Every reward
    // write below defends its own field for the same reason.
  ...
  if (isFinalChapter && chapter.onComplete) {
    ...
    // Same defence as the chapter gems above, and it matters more here: this is
    // the volume's payoff (character + items + perks), the one moment a throw
    // would cost a reader all 20 chapters.
